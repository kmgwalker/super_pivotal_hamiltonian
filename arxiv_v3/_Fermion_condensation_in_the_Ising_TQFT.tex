...
An important consequence of the existence of the back wall is that the quotient category $C_2$ 
...
This ``bosonizes'' the $\psi$ endpoints allowing the condensation process to go through. 
...
More specifically, for each string net $S$ in $M$, we have an endpoint configuration $e(S) \in \mcr(B)$.
...
First, we observe that any $\psi$ strands can be absorbed into the 
...
More generally, the vector space assigned to a disk with $2n$ $\beta$ strings ending on its boundary 
