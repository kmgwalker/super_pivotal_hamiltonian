...
\dave{This one \cite{Joyal1991}? (I haven't read it).}
...
\dave{Will add comment about S-matrix giving Frob-Schurr indicator.}
...
%\ethan{done}
\dave{Noticed some $\mcx$'s in this section.}
...
We require that $r$ is compatible with the other structure maps of $\spc$ (pivots, $F$, etc.).
...
The ``identity" map from $Y$ to $-Y$ is not a spin diffeomorphism (it reversed orientation), but it is a pin+ diffeomorphism.
...
\kw{It will take more work to show that the hamiltonians of sec 9 are hermetian; if we do that work it should probably
go in sec 9}
