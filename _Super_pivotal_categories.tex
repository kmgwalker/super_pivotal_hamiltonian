...
\be
	K_{fg} : T_f \to T_g ,
\ee
characterized by\footnote{\ethan{added the following to dispel some confusion:} We should mention that in this section, 
the left-to-right ordering of tensor factors appearing in equations is tied to their Koszul ordering only, and
are independent from the order in which they appear when written down in diagrams. This is in contrast to several other points in the 
...
Let $\sob(\scat)$ be a complete collection of simple objects (minimal idempotents) in some 
input super fusion category $\scat$, one from each equivalence class.
(In this subsection, as in most of the paper, we are assuming that our category $\scat$ is semisimple with finitely many equivalence
classes of simple objects.)
For arbitrary objects $x$ and $y$, we have
\be \label{sobdecomp}
	\mor(x\to y) \: \cong \: \bigoplus_{a\in\sob(\scat)} \mor(x\to a) \tp_{\End(a)} \mor(a\to y) .
\ee
...
Our semisimplicity assumption implies that the composition map (summing over all $a\in \sob(\scat)$) is surjective and that such
...
\be \label{pfi1}
	V^{abc}_d \cong \bigoplus_{x\in \sob(\scat)} V^{ab}_x \tp_{\End(x)} V^{xc}_d
\ee
and also
\be \label{pfi2}
	V^{abc}_d \cong \bigoplus_{y\in \sob(\scat)} V^{ay}_d \tp_{\End(y)} V^{bc}_y .
\ee
...
Similarly, 
\be
	V^{abcd} \cong \bigoplus_{x\in \sob(\scat)} V^{ab}_x \tp_{\End(x)} V^{xcd} ,
\ee
and using the isomorphism $P_R: V^{ab}_x \to V^{abx^*}$ this becomes
\be  \label{pfpfi1}
	V^{abcd} \cong \bigoplus_{x\in \sob(\scat)} V^{abx^*} \tp_{\End(x)} V^{xcd} .
\ee
%\kw{need figs
...
Alternatively,
%\be  \label{pfpfi2}
%	V^{abcd} \cong \bigoplus_{x\in \sob(\scat)} V^{bc}_x \tp_{\End(x)} V^{axd} \cong \bigoplus_{x\in \sob(\scat)} V^{bcx^*} \tp_{\End(x)} V^{xcd} .
%\ee
%\dave{Changed so that they only differ by a pivot like the above.}
\be  \label{pfpfi2}
	V^{abcd} \cong \bigoplus_{x\in \sob(\scat)} V^{bc}_x \tp_{\End(x)} V^{axd} \cong \bigoplus_{x\in \sob(\scat)} V^{bcx^*} \tp_{\End(x)} V^{axd} .
\ee
...
\be  \label{fdef21}
	F^{abc}_d : \bigoplus_{x\in \sob(\scat)} V^{ab}_x \tp_{\End(x)} V^{xc}_d \;\to\; \bigoplus_{y\in \sob(\scat)} V^{ay}_d \tp_{\End(y)} V^{bc}_y .
\ee
For the pitchfork version, we instead use \ref{pfpfi1} and \ref{pfpfi2} to obtain
\be  \label{fdef30}
	F^{abcd} : \bigoplus_{x\in \sob(\scat)} V^{abx^*} \tp_{\End(x)} V^{xcd}   \;\to\;   \bigoplus_{x\in \sob(\scat)} V^{bcx^*} \tp_{\End(x)} V^{axd} .
\ee
...
\be \label{Fmove_ordering_conventions}
	F^{abc}_d : \bigoplus_{x\in \sob(\scat)} V^{xc}_{d} \tp_{\End(x)} V^{ab}_{x} \;\to\; \bigoplus_{y\in \sob(\scat)} V^{ay}_{d} \tp_{\End(y)} V^{bc}_{y} ,
\ee
...
%\be \label{Fmove_ordering_conventions}
%	F^{abc}_d : \bigoplus_{x\in \sob(\scat)} V^{ab}_{x,2} \tp_{\End(x)} V^{xc}_{d,1} \;\to\; \bigoplus_{y\in \sob(\scat)} V^{ay}_{d,1} \tp_{\End(y)} V^{bc}_{y,2}.
%\ee
...
and where the first sum is over $y\in \sob(\scat)$. 
We also stick with the left-to-right ordering convention for the $F^{abcd}$ move:
%\dave{Changed order and and $x \ra x^*$.}
\be  
	F^{abcd} : \bigoplus_{x\in \sob(\scat)} V^{abx} \tp_{\End(x)} V^{x^*cd}   \;\to\;   \bigoplus_{x\in \sob(\scat)} V^{axd} \tp_{\End(x)} V^{x^*bc} .
\ee
...
\begin{align}
\label{idpitchfork}
\Idabba = \sum_{x} \frac{d_x}{B(\mu \tp \mu^* )} \IdPitchfork
\end{align}
