...
>>>>>>> cb197e0dee1c2839cf5c8ee4fef697ac6ee2a84d
...
\subsubsection{Tensor product in $\cal{C}/\psi$}
...
\label{Exampletp}
\end{align}
We have denoted an equivalent state on the right by sliding the dot past the junction. 
This indicates that we should modify our tensor product to account for this equivalence relation.
...
Indeed, the analog of shuttling the dot past the junction is shuttling a complex number from one vector to the other
\begin{align}
...
In doing so we have to allow for arbitrary pictures to be shuttled past the gluing junction. 
Shutteling a picture past the junction is just an endomorphism of the boundary condition. 
Hence \dave{will clean up when we decide how to present.},
\begin{align}
P_{a} \tp_{\text{B.C.}}P_b= (P_a, P_b) \quad \text{such that} \quad (P_a, \text{End}(B.C.)P_b) \sim (P_a,\text{End}(B.C.) P_b)
\end{align}
Hence we modify the tensor product in (\ref{Exampletp}) to a tensor product over the endomorphisms of the boundary, which in this case is the simple object $\beta$,
\begin{align}
\FubeXss \otimes_{\text{End}(\beta)} \FubeXsds  \equiv \left(\FubeXss, \FubeXsds \right) / \sim \\
\text{where} 
\left (\FubeXss \text{End} (\beta), \FubeXsds \right) \sim    \left(\FubeXss,  \text{End}(\beta) \FubeXsds \right)
\end{align}
We are free to choose a single representative of this equivalence class which, for example, 
\begin{align}
...
We will re-visit this observation when we discuss the $F$-symbols of the $\cal{C}/\psi$ and later provide an explicit basis for this.

\subsection{F-symbols}
The F-symbols provide a linear relation diagrams.
Using the F-symbols from the original Ising theory, one can simply infer the linear relations the diagrams in the condensed theory. 
...
More formally, the F-symbol is a change of basis on the Hom space $V^{abc}_d$ \dave{explained in intro to Ising TQFT?}
\begin{align}
F^{abc}_d: \oplus_x V^{ab}_x \tp_{\text{End}(x)} V^{xc}_d \rightarrow \oplus_y V^{ay}_d \tp_{\text{End}(y)} V^{cd}_y
\end{align}
It is useful to choose an explicit basis for the splitting spaces and write down the F-symbols explicitly.
To that end we define the basis $e^{ab}_{c;\alpha} \in V^{ab}_c$. Where the index $\alpha$ denotes the parity of that splitting space. 
\dave{Perhaps we should define tensoring over $\text{End}(\beta)$ as projecting onto some subspace?}
...
Fusion cateogories are integral to the study of a topological phases.....
...
- picture of points on a line/idempotent complete: objects are associated with idempotents and simple objects with minimal idempotents. pictures.

- Simple objects are defined by there endomorphism algebra. 
In an ordinary fusion category this is just $\mathbb{C}$. 
This is because any simple algebra over $\mathbb{C}$ is Morita equivalent to $\cal{C}$. 
...
This algebra is generated by two elements, one even call it $1$ and odd $\lambda$. 
Where $\lambda^2 =1$.
A generic element $x$ the algebra $Cl(1)$ can be written as $x = \alpha 1 + \beta \lambda$ with $\alpha, \beta \in \mathbb{C}$. 
...
The simple objects of the theory can fuse together. 
