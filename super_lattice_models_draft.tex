...
A useful picture for this condensation process is to imagine the existence of a codimenion-1 ``back wall'' that plays the role of the condensate. 
...
The above tubes form a complete basis for the tube algebra in the bosonic case. In the fermionic case however, each tube must come equipped with a spin structure, given by an element $\eta \in H^1(C,\zt) \cong \zt$, where $C$ is the cylinder. $\eta$ simply determines the nature of the boundary conditions for fermions travelling around the non-contractible cycle of the cylinder, and so the boundary conditions for tubes must obey a $\zt$ group structure. We have to be a little careful about making this identification, though: contractible fermionic loops that twist fermion ribbons by $2\pi$ are associated with a $-1$ sign, and so the identity element in $H^1(C,\zt)$ corresponds to {\it anti-periodic} (or non-vorex) boundary conditions, with the nontrivial element corresponding to periodic (or vortex) boundary conditions.
...
\end{align}
All other tubes turn out to be nonzero for both spin structures, and so a complete basis for tubes in the non-vortex sector is given by
\begin{align}
...
\dave{Why push this off to the appendix? I thought it was one of the purposes of this paper.} \ethan{Okay fine. I reserve the right to bitch about it later though \smile} 
...
\dave{I don't understand why this is a consequence of Schur's lemma. I thought the definition of a simple object was $\text{End}(x) = \mathbb{C}$} \ethan{The definition of a simple object is that it can't be written as the direct sum of other objects --- i.e. if we think of objects as vector spaces, then a simple object is one with no proper subspaces. The Shur's lemma tells you that for simple objects, everything in $\End(x)$ must be an isomorphism, and so $\End(x)$ must be a division algebra. $\cc$ is the only ungraded division algebra, so normally being simple is the same as having $\End(x) = \cc$. But in the graded case, $\cl_1$ is also a division algebra.} 
...
\dave{Oops! Good spotting. They should be there.} \ethan{how should we deal with them? Define a canonical ordering for the vertices in the $F$-moves and write out all the $(-1)^{|a||b|}$s, or just write some $P(x,y,\dots)$ factor and keep it implicit like KW was suggesting?}
%\begin{align}
