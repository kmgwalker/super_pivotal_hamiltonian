...
\end{align}
for all $f \in \text{End}(x)$. \ethan{I just used multiplicative notation, which seems fine to me since the $f_i$s are just multiplication by numbers.}
\dave{I'm fine with the notation. But why do you say that $f_i$ is just multiplication by a number? It's an odd operator so at the very least it changes the parity of the state.} \ethan{right, should have said Grassmann number (not c-number)}
...
A fermionic phase of matter will generate such terms at second order in perturbation theory when proximity coupled to a superconductor.
