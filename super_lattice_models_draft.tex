...
\section{Majorana Fibonacci}
\dave{We could try to compute this. It doesn't seem so unreasonable any more. We would assume $\text{End}(\tau) = \mathbb{C} \ell_1$. And that $V^{\tau \tau}_\tau = \mathbb{C}^{1|1}$. It's possible that there are no invertible solutions to the pentagon equation, but not clear yet. Fermions sliding past vertices can only ever give $\pm 1$. }
 \section{Hamiltonian} 
 \begin{figure}
...
\dave{Add interesting fact about sliding dots past fusion vertices. Can show that if sliding a dot up to the left past $V^{ab}_c$ takes $V^{ab}_c \rightarrow U V^{ab}_c$ then $U = U^{-1}$. Similarly sliding a dot from upper left to upper right has a consistency condition that $U_{L\rightarrow R}^{-1} = -  U_{L\rightarrow R}$. As a sanity check this is true for half-E6.}
...
\end{align}
\dave{add pivoting in other direction.}
