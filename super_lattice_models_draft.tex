...
\dave{Good point. But I think that's implied by them being able to slide along the lines. We could add some more pictures though.}
 ...
 \end{align}
A tensor product taken over a Q type particle must be quotiented by these isomorphisms. That is, \ethan{This is just modding out by the action of $L_f$. Should we also mention that we need to mod out by $R_f$? Not sure}
\dave{You're right, I think we should, maybe just repeat the equation with the other fusion space.}
\begin{align}
...
%\end{align}
\begin{align}
V^{ab}_{x,1} \tp_{\text{End}(x)} V^{xc}_{d,2} \equiv V^{ab}_{x,1} \tp_{\mathbb{C}}V^{xc}_{d,2}/ \sim,
\label{qtensor}
\end{align}
where $\sim$ is the equivalence relation 
\begin{align} \label{equivreln}
(V^{ab}_{x,1} \circ f_3) \tp_{\mathbb{C}}V^{xc}_{d,2} \sim V^{ab}_{x,1} \tp_{\mathbb{C}} (f_3\circ V^{xc}_{d,2}) 
\end{align}
for all $f \in \text{End}(x)$. \ethan{why do we have the $\circ$ symbol? Are we really composing a morphism with a vector?}
\dave{At some point I thought it would be more clear. But maybe we should replace it with $\tp$. The only annoying thing is that we've overloaded $\tp$ a little bit.} This allows us to write down the correct version of \eqref{resid} as
\begin{align}
