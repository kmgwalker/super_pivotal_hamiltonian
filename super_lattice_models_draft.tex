...
Define super fusion category for target audience.
...
As we saw in the Ising example, the endomorphism algebra is not always simple, it can be a simple super algebra over $\mathbb{C}$. 
As is the case for the $\beta$ idempotent.
All simple super algebras are Morita equivalent to the clifford algebra with one odd generator $Cl(1)$\cite{wall1964}.
This algebra is generated by two elements, one even call it $1$ and odd $\Gamma$. 
Where $\Gamma^2 =1$.
...
We will denote these simple objects as a Q type object.
In summary we have 
\begin{align}
& \text{End}(x) = \mathbb{C}  \quad  \leftrightarrow \quad  \text{x is a simple M type object}  \\
&\text{End}(x) = \mathbb{C} \ell_1  \quad \leftrightarrow  \quad \text{x is a simple Q type object}
\end{align}
...
A defining feature of a simple object is its quantum dimension
\begin{align}
...
The set of input date for a super fusion category will not only constitute a list of simple objects, there types (Q or M) but also there fusion rules.
...
\end{align}
If any of the particles are Q type, one always has a $\mathbb{C}^{p|p}$ dimensional vector space.
...
\dave{Mention how M type paritcles that have the same number of even and odd fusion channels with themselves is very different from Q type.}
The fact that the fusion vertices involving $Q$ type particles allow fermions to freely enter and exit is reminiscent of superconductivity. 
The odd endomorphisms allow the parity of the fusion space to fluctuate. 
This is reminiscent of superconductivity. 

Taking tensor products of these fusion spaces comes with two subtelties. 
Since each fusion space is a super vector space, we have give them an ordering, so each fusion space is equipped with an integer that denotes its placement in an ordering of all the fusion spaces. 
This is of paramount importance since in a fermionic theory the splitting spaces could be odd or even, and interchanging the two indices may result in the interchange of two fermions, for which we would pick up a minus sign. 
Thus the fusion category has a canonical braiding inherited from the fermions given by:
...
This is just the sign due to fermion ordering. 

The other subtelty pertains to the Q type particles, they have odd endomorphisms means that tensor products involving Q type particles have extra local relations on them given by shutteling a fermion from one fusion vertex to another. 
Hence a tensor product taken over a Q type particle should be quotiented by these isomorphisms:
\begin{align}
&V^{ab}_{x,2} \tp_x V^{xc}_{d,1} \equiv V^{ab}_{x,1} \tp_{\text{End}(x)} V^{xc}_{d,1} \\
&\equiv V^{ab}_{x,2} \tp_{\mathbb{C}}V^{xc}_{d,1}/\langle (V^{ab}_{x,2} \circ f_3) \tp_{\mathbb{C}}V^{xc}_{d,1} \sim V^{ab}_{x,2} \tp_{\mathbb{C}} (f_3\circ V^{xc}_{d,1}) \rangle \quad \text{for all} \quad f \in \text{End}(x)
\label{qtensor}
\end{align}
...
\subsection{Tensor products in super fusion categories}
...
Consistency with pivoting \dave{Need to place this some where better.}
\begin{align}
