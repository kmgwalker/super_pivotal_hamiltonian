...
The fact that the fusion vertices involving $Q$ type particles allow fermions to freely enter and exit the fusion space is reminiscent of superconductivity. \ethan{might need to explain this to me...} \dave{Not saying anything fancy. Just that pairs of fermions hopping on and off of the diagrams for free is what happens when you bring an S-wave superconductor nearbye an electronic system. So in a heuristic sense it looks roughly like we've taken a conventional super-fusion category and induced pairing on it through e.g., proximity effect.}
...
\end{align}
The above relation is actually {\it incorrect} in fermionic settings, since if $x$ is simple then this reads $\Hom(x,x) = \Hom(x,x) \tp_\cc \Hom(x,x)$, which since $\End(x) = \cl_1$ implies $\cl_1 \cong \cl_1\tp \cl_1 \cong \cl_2$, a contradiction. In order to fix this, we need to mod out by ``internal'' degrees of freedom carried by Q type particles. 

Graphically, modding out by these internal degrees of freedom is equivalent to modding out by local relations involving Q type particles. These relations are given by locally creating pairs of fermions and by sliding fermions from one fusion vertex to another: \ethan{do we want another picture for fermion sliding? It's probably not strictly necessary, as long as we mention that dots can slide through vertices}
\dave{Good point. But I think that's implied by them being able to slide along the lines. We could add some more pictures though.}
 \begin{align}
...
\be \ba
 &V^{ab}_{x,1} \tp_{\mathbb{C}} V^{xc}_{d,2}\quad  \text{is replaced by} \quad V^{ab}_{x,1} \tp_{\text{End}(x)} V^{xc}_{d,2}, \text{ and} \\ 
  &V^{ab}_{x,1} \tp_{\mathbb{C}} V^{xc}_{d,2}\quad  \text{is replaced by} \quad V^{ab}_{x,1} \tp_{\text{End}(x)} V^{xc}_{d,2},
\ea 
\ee
