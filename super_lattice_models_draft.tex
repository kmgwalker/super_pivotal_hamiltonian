...
- Introduce graphical calculus for Ising TQFT \cite{Lins1994}
...
It will depend on whether we choose a $\text{Pin}^{\pm}$ structure. 
For now we pick $\lambda = i$. 

\subsection{Graphical calculus of super Ising }
...
We first observe that any diagram with a $\psi$ line can be reduced to one where all the $\psi$ lines terminate on the spin structure defect. 
In the target theory the only objects remaining will be $\sigma$ lines with possible fermions attached. 
