...
\caption{A complete basis of morphisms $\mor(\rho \ra e) \in  \tube(\halfesix) $ and $\mor(e \ra \rho) \in  \tube(\halfesix) $.
...
We start by analyzing the idempotents from a purely algebraic point of few using only the dimensions of the morphism spaces which can be inferred from tables \ref{etoe}--\ref{rhotorho}.
...
and (as we have seen before \ref{C2etoe}) there is only one possible super algebra structure on $\cc^{2|0}$ given by $\cc \oplus \cc$ (as before $\cc$ is shortand for the 1-dimensional algebra).
...
which a basis is listed in table \ref{etorho}. 
...
Naturally we now look at $\mor^B(\rho \to \rho)$ and ask which of the generators have not been accounted for. 
...
and thus rules out $\cc^{3|3}$ arising from three copies of $Q(1)$.
...
We also list the quantum dimensions of the idempotents in Table \ref{TubehalfesixQD}.
...
As in Section \ref{mtc_idem_subsect} the inclusion can be non non-trivial, for example some of the idempotents may become isomorphic, or even equal to zero.
...
\begin{flalign*} & \begin{array}{r@{ \quad \quad \quad}  c @{\quad \quad} c @{\quad \quad} c @{\quad \quad \quad \quad } c  @{\quad \quad} c @{\quad \quad} c  }
	...
	We have normalized the quantum dimensions by the quantum dimension of the trivial idempotent. 
	 ...
	 \dave{This label has been used multiple times, need correct that and fix referencing.}}
...
\Pantsmsixc &= \sum_{\alpha \beta} X_{a\alpha} (X^2)_{b \beta} \Pantsmsixd.
\end{align}
\dave{We should re-define $X$ to $P$ since it is a pivot, and we use $X$ for many things in this section.
I'll do this soon.}
...
\end{align}
Under interchanging $X_{11}$ with $X_{12}$ we have $m_6$ is interchanged with $\Pi(m_6)$. 
...
Using that $\overset{\bullet}{v}\cdot  \Pi(m_6) = m_6 \cdot \overset{\bullet}{v}$, a basis for $V^{m_6 m_6}_\unit$ is generated by, 
\begin{align}
\nonumber
V^{m_6 m_6}_\unit =   
&\left \langle \Vmsixa
