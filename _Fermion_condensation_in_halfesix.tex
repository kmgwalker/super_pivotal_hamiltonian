...
We start by analyzing the idempotents from a purely algebraic point of few using only the dimensions of the morphism spaces given in tables \ref{etoe}--\ref{rhotorho}.
Later, we will need explicit representations of the idempotents which we will also provide in tables \ref{MIdempotentsprime} -- \ref{QIdempotents}.
...
but we know that the bounding tube category not admit q-type simple algebras, see the discussion at the end of Section \ref{ground_states_on_torus}.
...
There is a unique simple algebra structure on $\cc^{1|1}$ given by $\cl_1$. 
We denote the corresponding q-type minimal idempotent by $q_1$.

Next we look at $\mor^N(e \to \rho) \cong \cc^{1|1}$ as a vector space. 
It follows that $\mor^N(\rho \to \rho)$ has a $Q(1)$ matrix algebra, and a minimal idempotent isomorphic to $q_1$. 
...
Hence we only need to look at the remaining $\cc^{3|3}$ dimensional matrix algebra. 
...
However, the number of non-isomorphic minimal idempotents of $\tube^B$ must is the same as $\tube^N$, as described at the end of Section \ref{ground_states_on_torus}. 
Since $\tube^B(\halfesix/y)$ has three non-isomorphic minimal idempotents, 
so must $\tube^N(\halfesix/y)$. 
This the remaining generators of $\mor(\rho \to \rho)$ must constitute two non-isomorphic minimal idempotents, and thus rules out $\cc^{3|3}$ arising from three copies of $Q(1)$.
Thus the remaining generators are accounted for by two matrix algebras $Q(1) \oplus M(1|1)$.
Corrspondingly we find one q-type minimal idempotent which we label $q_2$, 
...
$q_1$, $q_2$ and $m_6$.\dave{ refer to tables}
