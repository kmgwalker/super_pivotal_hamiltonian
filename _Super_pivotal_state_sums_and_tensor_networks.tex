...
%%%%%%%%%%%%%%%%%%%%%
List of modifications to incorporate: 
\begin{itemize} 
\item 3-manifold $M$ is replaced by spin 3-manifold
\item The state sum is given by 
\begin{align}
\label{fermionic_tv_sum}
	Z(M) = \sum_{\beta\in\mcl(\mch)}(-1)^{\kappa_\beta}
		...
		\prod_{v\in\mch_0} \text{Link}(v, \beta) .
\end{align}
The main modifications are given by the Koszul sign $(-1)^{\kappa_\beta}$, the normalization on the 2-handle term, and the sign ordering that is introduced on $\widetilde{\Theta}$ and $\text{Link}(v, \beta) $.
\item $d(f,\beta)$ is as before
