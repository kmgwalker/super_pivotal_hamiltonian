...
\dave{Should make this 1-handle a little wonkier.}
...
to the boundary of the disk ($k$ is the number of 2-handles which cross the 1-handle).
...
In other words, we choose (independently, without any compatibility constraints) a labeling of the edges of each initial
disk graph by simple objects, then choose a basis vector for each associated vector space.
...
There is a bilinear pairing between $V(\ell)$ and $V^*(\ell)$, found by applying the standard bilinear pairing \eqref{reflection_pairing_defn} after a rigid translation through the 1-handle.
...
We are finally ready to define the weights appearing in the state-sum $Z(M)$. 
...
If they are all equal to the same simple object $a$, we define the weight $d(f, \beta)$
...
corresponding 1-handle if it is on the initial (terminal) end of the 1-handle.\footnote{The dual spaces here are determined by the bilinear pairing of \eqref{reflection_pairing_defn}.}
...
If follows from Section 8.2 of \cite{Walker2006} that the state sum computes $Z(M)$, independently of the choice of handle decomposition
...
where the tensor trace tTr denotes the tensor contraction.
The tensor contraction introduces a factor of $\widetilde{\Theta}^{-1}$ if the labels on each side of the 1-cell are dual to one another (with respect to the bilinear pairing) and is zero otherwise.
...
The partition function is still performed by contracting, which contracts out the tensor factors in $\mch^*_{bulk} \tp \mch_{bulk}$ but leaves those in $\mch(\partial M)$ uncontracted, producing a wavefucntion supported on $\mch(\partial M)$ (that is, a map $A(\partial M) \ra \cc$).
...
For example, depending on the choice of 1- and 2-cell orientations, there will still be eight possible vector spaces associated to 1-handles,
...
adjacent 3-cells agrees with the orientation of $M$\dave{I'm confused, how can the 2-cells have an orientation that agrees with $M$? (the 2-handles can but then the sentence isn't making a lot of sense.)}.
...
If $Y$ is a torus, we might hope that the network has translational symmetry, but generically this is not compatible with the global ordering trick.
...
In this convention, the $T_v$ tensors all take the form \ethan{I guess we don't really need the numbers}
\begin{align} 
T_v = \TetrahedronPrime \; \cdot \alpha \tp \beta \tp \gamma \tp \delta, 
\end{align}
where have defined the tensor weight by its evaluation of the picture on the right with 
...
edge orientations.  
Since there are $2^6 = 64$ possible choices of edge orientations, there will be 64 
...
This procedure results in nets on 0-handles which are all standardized independently 
of one another. 
This means that the form of a given trivalent vertex at the initial edge of a 1-handle $e$
will not generically be the same as the form of the associated vertex on the terminal edge of the 1-handle:
the two may be related by a pivot operation $P_e=P^{l_e}$ for $l_e = 0,1,2$, where $P$ is the pivot defined in \eqref{Pitchfork_pivot}.
...
\dave{Should probably say what the standardized partition function is. 
Here is a sloppy attempt at that. 
Need to improve it though.}
By standardizing the 0-handles, we have been forced to put extra data on each 1-handle, namely the pivots. 
The 1-handle terms in the partition function (tensor trace) also need to be modified.
...
We now turn to the generalization of the state sum construction to the fermionic case. 
We start with two pieces of data: a super pivotal fusion category $\spc$, and an oriented spin 
...
In turn, the partition function is weighted by a Koszul sign $(-1)^{\kappa_\beta}$, that compensates for the sign ordering attached to each of the weights
...
\item The spin structure on $M$ is determined by the spin structure on the 1-handles, and correspondingly one needs to account for the possible application of the spin flip functor when rigidly translating the vector space from the initial end of the 1-handle, 
to the terminal end.
\end{itemize} 
In the manner of the discussion in the previous section, we will now explain and motivate the factors appearing in \eqref{fermionic_tv_sum}. 
...
The 2-handle weight is the given by the bilinear pairing determined by the a given 1-handle $e$. 
...
The spin structure of the initial and terminal disks may differ by a spin flip functor. 
Hence the 1-handle term weight is given by $(-1)^{F(e,\sigma, \beta)}/\widetilde{\Theta}(e, \beta)$. 
Where $\sigma$ is the spin structure, $\beta$ the labeling of the initial disk, and $F(e, \sigma, \beta)$ is $0$ if $\beta$ has even fermion parity, and $1$ if $\beta$ is odd, 
and the initial and terminal disk differ by a spin flip functor relative to the ``standard" pairing.
\dave{May want to re-word this. 
I want to keep the $(-1)^F$ outside of the $\Theta$ symbols, since that is more familiar to tensor network people.}
...
Finally, for each 0-handle $v$, $\text{Link}(v,\beta)$ is defined in the same way as in the bosonic case (namely, as the evaluation of a net determined by the 1-handles incident on $v$), 
although now for the evaluation of $\text{Link}(v,\beta)$ to be defined we need to assign an ordering to the vertices of $\text{Link}(v)$.
...
\dave{This is still in progress, so read with caution.}
Lastly, we need to define the over Koszul sign $(-1)^{\kappa_\beta}$, whose sole job is to compensate for the arbitrary sign ordering we've given each of the vertices on the 0-handle graphs $\text{Link}(v)$. 
...
\end{align}
Where $\sigma$ is the permutation in order to have the two global orderings match (equivalently, $\text{sign}(\sigma)$ is the number of transpositions needed for the two sign orderings to match modulo two). 
Additionally, two global orderings of the 0-handles (1-handles) are related by an even number of transpositions in the sign ordering, and so the sign $(-1)^{\kappa_\beta}$ is independant of any choice of global ordering.
Furthurmore, it is independant of the arbitrary ordering assigned the vertices of the 0-handle graphs $\text{Link}(v)$.
To see this, we note that if we change the ordering othe vertices of $\text{Link}(v)$ by a single transposition, then the weight itself will pick up a negative sign (if both labels are odd), 
...
\dave{I am skeptical of the following, but wanted to write down the idea while I was thinking about it.
}
We also note that $(-1)^{\kappa_\beta}$ can be written as the evaluation of a diagram in $\text{sVec}$. with simple objects $\unit$ and $\psi$ satisfying the usual fusion rules, and exchange statistics.
This has the advantage of implicitly modding out by even permutations, and does not require global of the 0- and 1-handles. 
...
The Koszul sign $(-1)^{\kappa_\beta}$ written as the evaluation of a diagram in $\text{sVec}$. 
Solid lines are labeled by $\psi$ and dashed lines by the trivial object. 
On the left we have written two 0-handles $\text{Link}(v)$ and $\text{Link}(v')$, as well as there associated 1-handles that connect them.
}
\end{figure}
The evaluation of the diagram can be written as a TVBW state sum, but since we have written the evaluation on the plane, it doesn't sit naturally with the TVBW state sum above. 
\dave{I suspect that: $(-1)^{\kappa_\beta}(-1)^{F(e, \sigma,\beta)}$ can be written as a TVBW state sum on $M$.
In which case we can write $Z(M) = \sum_{\beta \in \mcl(\mch)} Z(\mcc, M,\beta) Z(\text{sVec},M, \beta)$.
Maybe this is Kapustins shadow thing?}
...
In order to perform the contraction, the we require the pair of vectors to be contracted to be in adjacent Koszul ordering (with respect to the pairing).
\dave{Need to word previous sentence better.}
To get the pair of vectors in adjacent Koszul ordering we need to apply a number of Koszul isomorphisms. 
After contracting all vectors we pick up a factor of $(-1)^{\kappa_\beta}$. 
\dave{Probably need to re-word those sentences a bit.}
Again, $Z(M)$
is independent of the way we assign factors of $d(f,\beta)$ and $\mcd^{-2}$ to the 
vertex tensors. 

\dave{I think the following can be condensed into a few sentences (I have made a first attempt at) above.}
However, 
...
valued in $\zz_6$ as opposed to $\zz_3$ (alternatively, we could keep them $\zz_3$ valued but insert a $(-1)^{F(e,\sigma)}\circ P_e^{l_e}$ on every 1-handle).
With the spin framing at the 0-handles fixed by virtue of the pitchforkization procedure (and every 1-handle related by a translation), 
...
(e.g., a sloppy (possibly incorrect) argument is to introduce a trivial branch sheet on every 0-handle and slide it over one of the attaching disks. 
