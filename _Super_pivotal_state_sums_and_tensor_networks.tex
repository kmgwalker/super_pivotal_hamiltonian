...
\kw{Can we replace a) with (a) etc?  The unbalanced parens confused me in the main text}
}
\end{center}
\end{figure}
...
The state sum turns out to be a version of the ``shadow world" state sum of \dave{Looks like the KR citation may need to be fixed: \cite{kirillow1989,turaev1992shadow}, maybe \cite{turaev1990} (need to look at that again)}.
...
Explicitly, for a 0-cell $v$ of $G$ consisting of three 1-cells of $G'$ (middle column of row (a) of Figure \ref{StandardizedSlabTensors}), 
...
\end{align}
\kw{Are we confusing vectors and dual vectors here?}
...
0-cell in the left column of row (a) in Figure \ref{StandardizedSlabTensors} involving three 1-handles from $G''$, we assign the tensor
