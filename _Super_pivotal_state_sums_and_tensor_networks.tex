...
\dave{This was taken from the old section.}
...
A 2-cell and its adjacent 0- and 1- handles. 
The oriented 2-cell determines a graph on each 0-, 1- and 2-handle. 
The graph is found at the intersection of the 2-cell with the 0- and 1-handles.
We have omitted the attachment regions that do not intersect the 2-cell.}
\end{center} 
\end{figure} 
...
There is a pairing between $V(\ell)$ and $V^*(\ell)$, given by \eqref{reflection_pairing_defn} and 
...
Diagrammatically, $\widetilde\Theta(e, \beta)$ is the evaluation of a banana-shaped diagram with vertices labeled
by $\mu$ and $\mu^*$.
...
\be \widetilde \Theta (e, \beta) =  \Bananafourmu.\ee
...
\dave{Should these be interchanged?
i.e., I thought $\mu^*$ is assigned to vertices of $\text{Link}(v)$ which sit at the initial end of a 1-handle.
}
...
\dave{I was confused by it.}

To incorporate the factors of $d(f, \beta)$ and $D^{-2}$, we make some ad hoc choices.
We consider ``dressed" 0-handle tensors that tack on the factors of $d(f,\beta)$ and $D^{-2}$.
For each 2-handle $f$ we choose an adjacent vertex $v_f$.
For a 0-handle $v$ we modify the associated tensor by added factors of $d(f,\beta)$ for each 2-handle $f$ such that $v = v_f$.
Similarly, for each 3-cell we choose an adjacent 0-handle and multiply the associated tensor by $D^{-2}$.
...
\dave{Are we still going to write the partition function as a tensor product over un-contracted tensors, 
and then contract?}
...
One way to reduce this irregularity is to start with a cell decomposition that is dual to a triangulation.
...
\dave{I think we need to discuss this earlier. 
I got confused when reading.
Were we implicitly assuming that the 1-handles were of the form $S^1 \times I$ above, and that the pivots have been pushed into the 0-handles?
(which is the same as cutting the 1-handles in the middle rather than at the attaching regions.)}
