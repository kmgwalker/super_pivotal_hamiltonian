...
We define the set of all labelings $\mcl(\mch)$ to be the product over all 1-handles $e$ of the basis sets $B(e)$, i.e., 
\begin{align}
\mcl(\mch) = \prod_{e \in \mch_1} B(e) 
\end{align}
\dave{Better notation? Figured we should display this so people will see it if they are only quickly scrolling through.}
...
\dave{Or should we change $\mcb$ to $\tilde{\Theta}$ and save $\Theta$ for trivalent junctions?}
...
\be \tilde \Theta (e, \beta) =  \Bananafourmu.\ee
%\ethan{need to change $\eta_i$ in the fig to $\mu^*$ and $\mu$}
%\dave{done}

\dave{Changed to 0-handle.}
Let $v$ be a 0-handle.
