...
\begin{itemize}
\item a spherical fusion category $\mcc$, 
\item and an oriented 3-manifold $\mcm$ 
...
possessing a cell decomposition with transversally oriented 2-cells 
\kw{We should think about whether a plain (non-transverse) orientation would be more convenient}
and oriented 1-cells. 
\end{itemize}
In what follows, we will assume that $\mcm$ is closed for simplicity. We will drop this 
...
3-manifold $\mcm$ is built from a series of $k$-handles, with $k=0,1,2,3$, each of which is identified with $D^k\times D^{3-k}$). 
...
and the attaching map of a $k$-handle is a homeomorphism from the attaching region to a submanifold of the boundary of the lower-dimensional handles.
The topology of $\mcm$ is encoded by the various attaching maps.
