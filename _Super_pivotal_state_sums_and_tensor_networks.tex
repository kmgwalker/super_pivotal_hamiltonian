...
We now define the 0-handle tensors $T_v$ as follows.
...
\ee
where $w_i\in V_i$.
In other words, $T_v$ evaluates the link graph with labels determined by $w_1,\ldots,w_k$.
\kw{... end ``as follows"}
...
using the state sum: 
\begin{align} 
Z(\Sigma_{0\ra1}; v_0, v_1) = \sum_{ \beta\in\mcl(\mch^{1/2})}  (-1)^{k_\beta} \prod_{f \in \mch^{1/2}_2} T_f(w_f) 
\prod_{e \in \mch^{1/2}_1} T_e(w_e) \prod_{v \in \mch^{1/2}_0} T_v(w_v),
\end{align} 
where we have used $\mch^{1/2}_k$ to denote the collection of $k$-cells in $\Sigma \times \{1/2\}$ and $\mcl(\mch)_{bulk}$ 
