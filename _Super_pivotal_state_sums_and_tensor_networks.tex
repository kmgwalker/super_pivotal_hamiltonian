...
\dave{A start on the figure: 
\begin{align}
\TetSphereColored
\end{align}
need to add orientations
Make into figure rather than equation?
Add other stuff?
}
...
Such an ordering is well-defined up to even permutations; see the upper row of Figure \ref{KoszulFig}.
...
parity at that 0-handle is odd); see the lower row of Figure \ref{KoszulFig}.
We then define $(-1)^{\kappa_\beta}$ to be Koszul sign relating these two orderings; 
see Figure \ref{KoszulFig} where for a diagrammatic definition.
...
\dave{Started making this caption, but too tired.}
The dashed circles are representing 0-handles (1-handles) on the bottom (top). 
For a 0-handle to have a well defined evaluation, the vertices if $\text{Link}(v)$ need to be ordered. 
This gives an sign ordering to each weight. 
$\kappa_\beta$ is given by the number of crossings.}
\end{centering} 
\end{figure} 
\medskip
