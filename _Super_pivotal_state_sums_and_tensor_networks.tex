...
\ethan{actually looks like we don't use Tet in any equations, though i'd be in favor of calling the $T_v$s Tets instead.}
\ethan{yes, we agreed to go and replace $T_v$ by Tet.}
\begin{align} 
\label{Bosonic_tet}
...
\ethan{Maybe. Should maybe also make reference to the $\alpha(e)$ things in the Hamiltonian section}
...
so that \ethan{replace with Tet}
\begin{align} 
	%T_v = \TetrahedronPrime \; \cdot \alpha \tp \beta \tp \gamma \tp \delta, 
