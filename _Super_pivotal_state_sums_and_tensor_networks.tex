...
In the left figure, we show the cell decomposition $G$ for a given simple choice of $G'$ (blue) and $G''$ (orange). 
In the central figure, we include the ``time direction'' in the picture, which thickens it into a box with boundary conditions 
set by $\bar g$ on the initial boundary $\Sigma \times \{0\}$ and boundary conditions set by $v$ on the terminal boundary $\Sigma \times \{1\}$. 
...
Instead of fixing a particular input string net $g$, we could put $g$ and $v$ on more equal footing by constructing
