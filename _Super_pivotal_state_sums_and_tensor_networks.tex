...
There is a bilinear pairing between $V(\ell)$ and $V^*(\ell)$,  %\eqref{reflection_pairing_defn} and 
...
and we will choose a dual basis of $V^*(\ell)$ such that the pairing matrix is diagonal.
...
Similarly, for each 3-cell we choose an adjacent 0-handle and multiply the associated tensor by $\mcd^{-2}$.
We then construct the vertex tensor 
...
\be Z(M) = {\rm tTr}\left(\sum_{\beta\in \mcl(\mch)}\bigotimes_{v\in \mch_0} T_v\right),\ee
...
\ethan{obviously still in progress}

When generalizing to the fermionic case, we will find it useful to employ a standardization 
procedure in which all trivalent vertices assume the ``pitchfork'' form introduced 
in our treatment of the Hamiltonian. 
\dots 
In this convention, the $T_v$ tensors are 
\begin{align}
...
After performing this standardization procedure, the forms of the 1-handles are modified 
to incorporate extra pivoting data. 
