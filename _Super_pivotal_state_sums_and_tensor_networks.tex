...
\be \label{bos_tv_sum}
	Z(M) = \sum_{\beta\in\mcl(\mch)}
		\prod_{c\in\mch_3} D^{-2}
		\prod_{f\in\mch_2} d(f, \beta)
		\prod_{e\in\mch_1} \tilde\Theta(e, \beta)^{-1}
...
as shown in Figure \ref{NotationStateSum}.
...
The orientation of the loop is determined by the orientation of the 2-cell. For a 2-cell 
$f$, this is shown in the top left of Figure \ref{NotationStateSum}. \ethan{may wan to remove the $c_{fe}$ stuff from the figure}
\item On 1-handles, the graph is a generalized $\Theta$ graph, which we will call a $\tilde\Theta$ graph.
...
For example, for the 1-handle drawn in Figure \ref{One_handle}, the left end of the 1-handle would 
...
to the boundary of the disk (which $k$ is the number of 2-handles which cross the 1-handle).
For each labeling $\ell$ \ethan{changed $b$ to $\ell$ since we use $b$ for simple objects all the time} of these edges by simple objects in $\sob(\mcc)$, we have an associated vector space $V(\ell)$.
...
We define the set of all labelings $\mcl(\mch)$ to by the product over all 1-handles $e$ of the basis sets $B(e)$.
...
There is a bilinear pairing between $V(\ell)$ and $V^*(\ell)$, which 
is computed as in \eqref{reflection_pairing_defn} and 
...
We choose a dual basis of $V^*(\ell)$ such that the pairing matrix is diagonal.
As with $\mcb$, it is sometimes convenient to not insist that the diagonal entries be $\delta_{ij}$.

We are finally ready to define the weights appearing the in the state-sum $Z(M)$. 
...
Define $\tilde\Theta(e, \beta)$ to be the value of the bilinear pairing evaluated on $\mu^*$ and $\mu$.
Diagrammatically, $\tilde\Theta(e, \beta)$ is the evaluation of a banana-shaped diagram with vertices labeled
by $\mu^*$ and $\mu$.
For example, if $e$ has three adjacent 2-handles colored by $a,b,c$ so that $\mu \in V^{abc}$, then we have 
\be \tilde \Theta (e, \beta) =  \Bananaeta.\ee
...
Near each vertex of $\text{Link}(v)$ we place the basis element $\mu$ (or $\mu^*$) assigned by $\beta$ to the 
corresponding 1-handle if it is on the initial (terminal) end of the 1-handle.
If these vertex labels are incompatible along edges, we define $\text{Link}(v, \beta) = 0$.
If they are all compatible then we define $\text{Link}(v, \beta)$ to be the evaluation of the resulting labeled graph (string net).
For a handle decomposition dual to a triangulation, these string nets take the form 
...
If follows from xxx of \cite{Walker2006} that the state sum computes $Z(M)$, independently of the choice of handle decomposition
and choice of orientations of 1- and 2-cells.

If $M$ has non-empty boundary,
we choose the handle decomposition to have the form show in Figure xxx near the boundary.
...
%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{The state sum as a tensor network}
%%%%%%%%%%%%%%%%%%%%%%%%%
...
If we (temporarily) ignore the factors of $d(f, \beta)$ and $D^{-2}$ in \eqref{bos_tv_sum}, it is easy to see that it computes the tensor contraction of a tensor network.
...
Because the vector spaces associated to the ends of each 1-handle are dual to one another, 
there are precisely as many dual vectors as vectors in the tensor product, and 
contracting each vector with its associated dual vector computes a complex number 
which we identify with $Z(M)$. 
\ethan{need to improve the wording of that last bit}

To incorporate the factors of $d(f, \beta)$ and $D^{-2}$, we make some ad hoc choices.
For each 2-handle $f$ we choose an adjacent vertex $v_f$.
For each vertex $v$ we modify the associated tensor by added factors of $d_a$ for each 2-handle $f$ such that $v = v_f$.
Similarly, for each 3-cell we choose an adjacent vertex and multiply the tensor for that vertex by $D^{-2}$.
It is easy to see that the resulting tensor network computes \eqref{bos_tv_sum} and 
is independent of the way we make such choices.

In the case where $\partial M \neq \emptyset$, the Hilbert space associated 
with the tensor product of the 0-cell tensors admits a decomposition $\mch^*_{bulk} \tp \mch_{bulk} \tp \mch(\partial M)$, where 
...
\ethan{need to improve working on the above bit}
...
One way to reduce this irregularity is to start with cell decomposition that is dual to a triangulation.
This ensures that there are exactly three 2-handles meeting each edge, and that the vertex graphs $\text{Link}(v)$ are all tetrahedral.
However, this still leaves us with many different types of $k$-cell string nets:
depending on the choice of 1- and 2-cell orientations, there will still be eight possible vector spaces associated to 1-handles,
corresponding to $V^{abc}$, $V^{ab^*c}$, $V^{a^*b^*c^*}$, etc.
(note that if all Frobenius-Schur indicators are 1, then we can ignore these differences).
...
\ee
Furthermore, all of the tetrahedral graphs have the same pattern of edge orientations (as in Figure xxxx), so all of the vertex tensors
in the tensor network can be identified.
...
If $Y$ is a torus, we might hope that the network has translational symmetry.
This is not compatible with the global ordering trick.
For this reason, we will assume a generic cell decomposition, but we will not employ the global ordering trick.

\kw{still need to discuss pitchforks and pivots ($V^{abc}$ vs $V^{bca}$);
should probably mention that standardizing the tetrahedra necessitates pivot relations between initial and terminal ends of 1-handles}
