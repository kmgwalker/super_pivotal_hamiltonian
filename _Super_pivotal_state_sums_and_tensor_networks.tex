...
In this subsection we construct a tensor network that produces the ground state wave function of the 
...
with similar definitions in the case of other edge orientation arrangements.
...
Let $G'$ be the cell decomposition of $\Sigma$ corresponding to $\mch_\mcg$, and let $G''$ be cell decomposition of $\Sigma$ underlying
the input string net $g$.
...
We define 1-skeleton of $G$ to be the union of the 1-skeletons of $G'$ and $G''$.
The cell decomposition $G$ has three types of vertices: vertices of $G'$ (which we will assume are 3-valent),
...
We have a 0-handle for each vertex of $G$, a 1-handle for each edge of $G$, and a 2-handle for each 2-cell of $G$.
...
Still to do:
\begin{itemize}
\item standardization or tetrahedra
\item operator version?? (if no, need to incorporate weights for 2- and 3-handles which straddle boundary)
\item if operator, explain why hermetian
\item say that resulting state sum is basically the ``shadow world"
\item what else?
\end{itemize}
