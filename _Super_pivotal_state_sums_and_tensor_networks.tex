...
A wave function (not ``the" wave function unless the ground state is 1-dimensional) $w$ assigns a weight $w(v)$ to each such basis vector $v$, \ethan{can we use $\Psi$ or $\psi$ instead of $w$, or am I being too picky?}
...
\end{align}
\caption{\label{slab_decomp}\dave{Should orient the edges and probably use different colors for $G'$ 
and $G''$ so it's easier to see how $G$ is made.} \ethan{I support the edge-coloring idea. maybe blue for $G'$ and orange for $G''$ to gel with previous figures?}
\dave{Sounds good.} 
\ethan{possible caption:} The handle decompositions described in the text. $G''$ (left) is the handle 
...
Figure xxxx\dave{aka, figure from your notes that we discussed?} \ethan{3d box with relevant net boundary conditions?}
\dave{e.g., somethign along the lines of \ref{VertexTypes}} illustrates this handle decomposition, and also shows how the string nets
$v$ and $\bar g$ are situated on its boundary.
\begin{figure}
\includegraphics[scale=0.6]{VertexTypes.pdf}
\caption{\label{VertexTypes}
\dave{A figure in progress.}}
\end{figure}
...
This is illustrated in Figure \ref{StandardizedSlabTensors} \ethan{placeholder fig for now} \dave{Could do this as a companion figure to the one above}, for the three different types of 0-handle in our handle decomposition $G$.
...
\ethan{talk about 1-cell and 2-cell things? write down it as state-sum rather than tensor network?}
