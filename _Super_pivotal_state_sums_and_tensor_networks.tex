...
Let $M$ be a closed 3-manifold equipped with a handle decomposition $\mch$.
...
For each labeling $b$ of these edges by simple objects in $\sob(\mcc)$, we have an associated vector space $V(b)$.
For example, in the case of fig xxxx the vector space is isomorphic to $V^{ab^*c^*d}$.
Let $B(b)$ be some chosen basis of this vector space.
For each 1-handle $e$ define $B(e)$ to be the union over all labelings $b$ of $B(b)$,
and also define $V(e)$ to be the direct sum of all the vector spaces $V(b)$.
...
We also associate a vector space $V^*(b)$ to the terminal disk of each labeled 1-handle.
In the example, this is isomorphic to $V^{d^*cba^*}$.
There is a (banana) pairing between $V(b)$ and $V^*(b)$.
We choose a dual basis of $V^*(b)$ such that the pairing matrix is diagonal.
(It is sometimes convenient to not insist that the diagonal entries be $\delta_{ij}$.)
We also define $V^*(e)$ to be the direct sum (over all labelings $b$) of $V^*(b)$.
...
If they are all equal to $a$, we define $d(f, \beta) = d_a$.

Let $e$ be a 1-handle.
The labeling $\beta$ associates a basis vector $\mu$ to the initial disk of $e$.
Define $\tilde\Theta(e, \beta)$ to be the value of the pairing evaluated on $\mu$ and $\mu^*$.
Diagrammatically, $\tilde\Theta(e, \beta)$ is the evaluation of a banana diagram with vertices labeled
by $\mu$ and $\mu^*$.
...
This can be interpreted as a linear function $A(\bd M)\to\cc$.
\kw{This might need some tweaking, or at least more details.}





\bigskip \kw{new subsection} \bigskip

Our goal in this subsection is to reinterpret \eqref{bos_tv_sum} as a tensor network and discuss various regularization options.

\medskip

If we (temporarily) ignore the factors of $d(f, \beta)$ and $D^{-2}$ in \eqref{bos_tv_sum}, it is easy to see that it computes a tensor network.
...
It is easy to see that the resulting tensor network computes \eqref{bos_tv_sum}.

\kw{discuss tensor network non-empty boundary case here}


\medskip

The above tensor network is irregular in the sense that (potentially) every edge vector space is different and every vertex tensor is different.
One way to reduce this irregularity is to start with a generic cell decomposition (one that is dual to a triangulation).
...
Similarly, there will be different tetrahedral vertex tensors, depending on the orientations of the edges of the tetrahedron.

We can further regularize the network by choosing a global ordering of the 3-cells of the handle decomposition.
