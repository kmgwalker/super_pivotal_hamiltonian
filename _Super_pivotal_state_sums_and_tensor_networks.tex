...
\kw{Some notational changes in next two paragraphs}
...
\kw{Maybe move the fancy right part to the bottom center, so that it can be larger}
}
\end{figure}
...
For a 0-handle $v$ of $G$, we define the tensors $T_v$ in the same way as in \eqref{Tv_defn}. 
The tensor weights are found by computing evaluations of string-net diagrams, as in previous sections. 
Explicitly, for a 0-cell $v$ of $G$ consisting of three 1-handles of $G'$ (left column of row $a)$ in Figure \ref{StandardizedSlabTensors}, we determine the entries of the tensor $T_v$ as 
