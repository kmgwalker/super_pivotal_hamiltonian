...
If the example, this is isomorphic to $V^{d^*cba^*}$.
...
If these simple objects are not all the same, we define $d(f, \beta)$.
...
or choice of orientations of 1- and 2-cells.
...
For each vertex $v$ we modify the associated tensor by added factors of $d_a$ for each $f$ such that $v = v_f$.
...
This ensures that there are exactly three 2-handles meeting each edge, and that the graphs $\text{Link}(v)$ are all tetrahedral.
