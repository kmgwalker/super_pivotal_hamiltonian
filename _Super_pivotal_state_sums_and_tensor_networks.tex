...
We stipulate that $G'$ and $G''$ are transverse \ethan{added the following:} (i.e., only their 1-handles intersect, and all the intersections are transverse), and are each dual to a triangulation, so that their vertices are all 3-valent.
...
\dave{... need to fill out caption.}
A handle decomposition for $\Sigma \times I$ (right) associated with the decomposition $G = G' \cup G''$ (left).
}
\end{figure}
...
This is illustrated in Figure \ref{StandardizedSlabTensors} \ethan{placeholder fig for now} \dave{Could do this as a companion figure to the one above}\dave{added fig. See \ref{StandardizedSlabTensorsprime}.}, for the three different types of 0-handle in our handle decomposition $G$.
...
Explicitly, for a 0-cell $v$ of $G$ consisting of three 1-handles of $G'$ (left column of row $a)$ in Figure \ref{StandardizedSlabTensors}, we assign the tensor $T_v$ as follows:
\begin{align}
...
0-cell in the center column of row $a)$ in Figure \ref{StandardizedSlabTensors} involving three 1-handles from $G''$, we assign the tensor
\begin{align}
...
where the tensor product runs over all 0-handles of $G$. 
