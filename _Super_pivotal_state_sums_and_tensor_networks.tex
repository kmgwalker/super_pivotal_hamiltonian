...
A wave function (not ``the" wave function unless the ground state is 1-dimensional) $\Psi$ assigns a weight $\Psi(v)$ to each such basis vector $v$, \ethan{can we use $\Psi$ or $\psi$ instead of $w$, or am I being too picky? \dave{That's not a bad idea.} cool, I made the change. Kevin, if you strenuously object we can change it back}
in such a way that if $\sum_i c_i v_i$ is equal to zero in $A(\Sigma)$, then $\sum_i c_i \Psi(v_i) = 0$.
For basis vectors $v$ which violate edge label compatibility, we have $\Psi(v) = 0$.

Given a string net $g$ on $\Sigma$, we can define a wave function $\Psi_g$ via
\be  \label{wfg-def}
	\Psi_g(v) = Z(\Sigma\times I)(\bar g \cup v) .
\ee
...
Note that as $g$ runs through a basis of $A(\Sigma)$, $\Psi_g$ runs through a basis of the wave functions for the ground state of the Hamiltonian, namely $\Psi_g  = \sum_{v \in \mch_\mcg}  \Psi_g(v) v$.
\ethan{maybe?}
...
We stipulate that $G'$ and $G''$ are transverse (i.e., only their 1-handles intersect, and all the intersections are transverse), and are each dual to a triangulation, so that their vertices are all 3-valent.
...
%\includegraphics[scale=.7]{CutUpG.pdf}
\caption{\label{VertexTypes}
Further illustrations of example cell decompositions. 
In the left figure, we draw the cell decomposition $G$ for a given simple choice of $G'$ and $G''$. 
In the central figure, we include the ``time direction'' in the picture, which thickens it into a box with boundary conditions 
set by $\bar g$ on the initial (orange) boundary and boundary conditions set by $v$ on the final (blue) boundary. 
...
while each tube shows a 1-handle. The green lines denote string nets in the interior of $\Sigma \times I$, which are not fixed 
...
Top row: different types of 0-cells that can appear in $G$. The cubes are drawn so that the 0-cells are  located in their centers. 
Blue (orange) lines represent 1-handles in $G''$ ($G'$), and 
...
\caption{\label{StandardizedSlabTensorsprime}
...
This is illustrated in Figure \ref{StandardizedSlabTensors} \ethan{placeholder fig for now} \dave{Could do this as a companion figure to the one above}\dave{added fig. See \ref{StandardizedSlabTensorsprime}.}, for the three different types of 0-handle in our handle decomposition $G$ (one of each type of 0-handle is shown in the rightmost picture of Figure \ref{VertexTypes}).
Note that our conditions on the handle decompositions $G',G''$ ensure that all three types of 0-handle string nets are tetrahedral (in that they each possess four trivalent vertices).

\medskip

We are now in a position to apply the state sum and tensor network constructions of the previous subsection.
The state sum turns out to be a version of the ``shadow world" state sum of \kw{[Kirillov and Reshetikhin; Turaev]}\dave{Looks like the KR citation may need to be fixed: \cite{kirillow1989,turaev1992shadow}, maybe \cite{turaev1990} (need to look at that again)}.
...
Explicitly, for a 0-cell $v$ of $G$ consisting of three 1-handles of $G'$ (left column of row $a)$ in Figure \ref{StandardizedSlabTensors}, we assign the tensor $T_v$ as follows: \ethan{the blue lines need to be curved up and left above the green thing to match the new conventions}
\begin{align}
\Tensora \; \ra T_v &= \;  \Tensoraa \cdot \mu_0 \tp s_1 \tp s_2 \tp s_3.
\end{align}
In the diagrams, the green letters $A,B,C$ denote the labels of the 2-cells in $G$, which will 
be summed over when computing the amplitude $Z(\Sigma\times I)(\bar g \cup v)$. The labels of 
the blue lines are fixed however, and are determined by the labels of the 1-cells 
in the string-net graph $v$. 

Tensors for the other types of 0-cells in $G$ are determined similarly. For the type of 
0-cell in the center column of row $a)$ in Figure \ref{StandardizedSlabTensors} involving three 1-handles from $G''$, we assign the tensor \ethan{the orange lines need to be curved down and right in the green thing to match the new conventions}
\begin{align}
...
Similarly, for the type of 0-cell in the last column (involving the intersection of 1-handles in $G'$ and $G''$) we assign the tensor
\begin{align}
...
where the tensor product runs over all 0-handles of $G$ (the evaluation of this tensor contraction involves the same 
treatment of Koszul signs and pivotal structure issues as the fermionic state sum discussed in the previous section) 
...
Instead of fixing a particular input string net $g$, we could put $g$ and $v$ on more equal footing and construct
...
Let $H$ denote the resulting tensor network operator. \ethan{Is this common terminology, or should we talk about it in terms of the Hamiltonian?}
