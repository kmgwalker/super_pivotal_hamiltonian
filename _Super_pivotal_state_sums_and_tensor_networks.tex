...
We first briefly review the TVBW construction for spherical fusion categories. 
We then show how to write the state-sum as a tensor contraction on a tensor 
network, which allows us to extend the state-sum to the super-pivotal case with only minor modifications.
...
Before we begin, we need to recall some terminology regarding cell and handle decompositions. 
...
A triangular 2-cell and its adjacent 0- and 1- handles. 
The oriented 2-cell determines part of the graphs on each 0-, 1- and 2-handle. 
The graph is found at the intersection of the 2-cell with the 0- and 1-handles.
...
There is a bilinear pairing between $V(\ell)$ and $V^*(\ell)$,
given by the 1-handle.
...
For a cell decomposition dual to a triangulation, these string nets take the form 
...
If follows from xxx\dave{Section 8.2?} of \cite{Walker2006} that the state sum computes $Z(M)$, independently of the choice of handle decomposition
...
If we (temporarily) ignore the factors of $d(f, \beta)$ and $\mcd^{-2}$ in \eqref{bos_tv_sum}, it is easy to see that it computes the tensor contraction of a tensor network.
...
We consider ``dressed" 0-handle weights that tack on the factors of $d(f,\beta)$ and $\mcd^{-2}$.
For each 2-handle $f$ we choose an adjacent vertex $v_f$.
For a 0-handle $v$ we modify the associated weight ${\text{Link}}(v,\beta)$by added factors of $d(f,\beta)$ for each 2-handle $f$ such that $v = v_f$.
Similarly, for each 3-cell we choose an adjacent 0-handle and multiply the associated weight by $\mcd^{-2}$.
...
which we identify with $Z(M)$. That is, we have 
...
where the tensor trace tTr denotes tensor contraction. 
...
Similarly, there will be many different tetrahedral vertex tensors, depending on the orientations of the edges of the tetrahedron.
...
outgoing and one incoming edge (Figure xxxx).
With these orientation choices, we have, for every 1-handle $e$,
\be
	V(e) = \bigoplus_{a,b,c\in\sob(\mcc)} V^{abc^*} .
\ee
Furthermore, all of the tetrahedral graphs have the same pattern of edge orientations (as in Figure xxxx), so all of the 0-handle tensors
...
If $Y$ is a torus, we might hope that the network has translational symmetry, but
this is not compatible with the global ordering trick.
...
in our treatment of the Hamiltonian (these vertices are all trivalent since we are now 
assuming a cell decomposition dual to a triangulation). 
In this convention, the $T_v$ tensors all take the form 
\begin{align}
T_v = \Tetrahedron \; \cdot \alpha \tp \beta \tp \gamma \tp \delta, 
\end{align}
...
\ethan{How do we want to incorporate the pivots into the state-sum? To me the best way to do this 
is via edge tensors in the tensor network construction like we had before. Maybe we don't need to mention it, though. }
...
We start with two pieces of data: a super pivotal fusion category $\mcc$\dave{Should we use $\spc$?}, and an oriented spin 
3-manifold $M$ possessing a cell decomposition with orientations of 1- and 2-cells.
The fermionic version of the state sum can be computed using similar techniques as in the 
bosonic case, with several important modifications:
\begin{itemize} 
\item Koszul signs enter into the partition function when computing it as a tensor contraction. In order to compute these Koszul signs, we need to assign an ordering to each of the vector spaces 
...
\item Since simple objects in $\mcc$\dave{$\spc$?} can have non-trivial endomorphism algebras, some additional care must be taken to ensure the proper normalization of the partition function. 
...
The set of all labelings $\mcl(\mch)$ is defined as the product over all 1-handles $e$ of the basis sets $B(e)$ together
with a choice of Koszul ordering for the ends of every 1-handle. Note that since each 
...
in order to ensure the proper normalization of the partition function, we need to divide
this weight by the dimension 
of the endomorphism algebra of the object that labels the loop of string appearing in the 
2-handle. This consideration is responsible for the factor of $n(f,\beta)$ appearing 
in the 2-handle weight, which is defined by $n(f,\beta)=\dim \End(a)$ for $f$ and $\beta$
...
We define the Koszul factor $(-1)^{\kappa_\beta}$ appearing in $Z(M)$ to be the sign picked up when 
permuting the given Koszul order into one for which the $\widetilde\Theta$ can be evaluated.
That is, $(-1)^{\kappa_\beta}$ is equal to $(-1)^{{\rm sgn}(\sigma)}$, where $\sigma$ is the 
...
Finally, for each 0-handle $v$, $\text{Link}(v,\beta)$ is defined in the same way as in the bosonic case, although 
now each vertex in the graph carries a Koszul ordering determined by the label set $\beta$.  
Of course, the 
sign of the evaluation of the graph depends on the choice of Koszul ordering.
\dave{It's not obvious to me that the 0-handle nets can be evaluated without applying permutations.}
...
However, in the fermionic case we now have $P_e^3=(-1)^F$, so $l_e$ is now 
