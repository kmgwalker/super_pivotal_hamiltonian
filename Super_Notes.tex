...
We can then calculate the braiding by moving the top punctures around eachother and re-expressing the result in terms of the basis vectors in $\fld^{b,a}_c$. When braiding $q$-type particles, we choose branch cuts that always meet in the center of the pants. For example, to calculate $R^{q_\psi q_{1/\psi}}_{m_{\psi/1}}$, we write
\be \includegraphics{qpsiq1psi_braiding} \ee
where we have taken advantage of our knowledge of the eigenvalue of $q_\psi$ under the twisted tube (i.e. the spin of $q_\psi$). 
...
%This thought process can also be used to say things about how to compute the modular transformations on the $(P,P)$ torus, where each of the linked tori in the previous figure possess $(P,P)$ spin structures and odd fermion parity. 
