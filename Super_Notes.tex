...
\title{First thoughts about fermions and an edit.}
...
The tables of values denote the row braiding over the top of the column, $e.g.$
\begin{tabular}{c|c}
     & B\\
     \hline
A     & $R^{AB}_{A\otimes B}$
...
\ethan{Also, using the formula for the $S$-matrix in terms of the twists gives us a $qq$ $S$-matrix of $e^{-i\pi/4}S_{Ising}$, which is what we ``should'' get if we expect to match up with Brayden's $S$-matrix.}
...
There are several ways to calculate $S$. For all of them, we should keep in mind that although there are some $\pm1$ ambiguities in $T$, such ambiguities should not be present in $S$, since fermions have trivial double-braiding with one another. 
...
If we try the supertrace instead, weighting odd fusion spaces by a factor of $-1$, we get 
...
Notice the factors of $\zeta^2$ on (almost) all the entries of the $qq$ sector! Since $\zeta^8 = -1$, this means that the restriction of the $S$ matrix to the $qq$ sector has a chance of satisfying the $S^4=-1$ relation that we know holds for the modular transformations on the torus with $(P,P)$ spin structure (more on this later). My present feeling is that a combination of trace and supertrace, with trace on the $mq$ sector and supertrace on the others, is what is correct. Also, more on this later. 

Instead of (super)tracing the $R$ matrices, we can appeal to a formula relating the $S$ matrix to the topological twists. In bosonic theories, we have 
\be S_{ab} = \frac{1}{\mcd} \sum_c d_c N^{ab}_c \frac{\theta_c}{\theta_a\theta_b}.\ee
This needs to be modified in the superfusion case. By looking at the derivation of this formula (might write up later), we see that it involves passing the fusion space $\fld^{a,b}_c$ under a $c$ tube and then over both an $a$ tube and a $b$ tube. When the braiding process is embedded in $S^3$, the branch cuts become branch sheets, and so passing an odd vector in $\fld^{a,b}_c$ under a $q$-type idempotent gives a phase factor of $-1$ (we could also choose this to be obtained when passing over a $q$-type idempotent). This means that we should modify the previous relation as 
\be S_{ab} = \frac{1}{\mcd} \sum_c \sum_{v \in \fld^{a,b}_c} d_c (-1)^{|v| |c|}   \frac{\theta_c}{\theta_a\theta_b},\ee
where $|v| \in \{0,1\}$ is the parity of the vector $v$ and $|c| = 1$ $(|c|=0)$ if $c$ is $q$-type ($m$-type). However, this cannot be completely correct, since it depends on our choice of $m_\psi = \pm1$, even though we argued that the $S$ matrix should not suffer from the $\pm$ ambiguities that $T$ possesses. This means that we should further modify the expression as 
\be S_{ab} = \frac{1}{\mcd} \sum_c \sum_{v \in \fld^{a,b}_c} d_c (-1)^{|v| |c| + \psi_m(v)}   \frac{\theta_c}{\theta_a\theta_b},\ee
where 
\be (-1)^{\psi_m(v)} = (-1)^{\delta_{a,m_\psi} + \delta_{b,m_\psi} + \delta_{c,m_\psi}}.\ee
The remaining question is whether or not there should be an additional $(-1)^{|v|}$ weight in the above expression. Essentially, this means that we have to decide whether to use the trace or the supertrace when calculating the $S$-matrix. A potential hint is that the correct modular $S$-matrix on the $PP$ sector is obtained through the formula
\be S_{PP}^{modular} = \frac{1}{\mcd} \sum_c\sum_{v \in \fld^{a,b}_c} d_c(-1)^{|v| + \psi_m(v)} [R^{ab}_c]_v[R^{ba}_c]_v = \zeta^2 S_{Ising} = S_{p+ip}\tp S_{Ising}.\ee
Note that this uses the supertrace and assigns an extra $-1$ to $m_\psi$ particles (presumably since we have defined them to carry an extra fermion). This will be updated later after I've thought about it a bit more. 

\subsection{Ground states on tori}

By drawing pictures, it's easy to see that spin structures uniquely determine the fermion parity of any given string-net picture drawn on the torus. We would then expect that each of the four tori would carry four degenerate ground states, since $|H^1(T^2)|=4$. However, for each spin structure one of these states will turn out to be of higher energy, meaning that there are only three degenerate ground states for each choice of spin structure. It will also turn out that the remaining degenerate ground states in each sector will all possess the same fermion parity. 
...
since the direct sum of a complete collection of primitive idempotents maps everything to itself. When we say ``compute ${\rm Tr}({\rm id})$'', we mean crossing the tubes in id with an $S^1$ so that they form a collection of tori, embedding them in $S^3$, and evaluating the resulting configuration by filling in the complement of the tubes with a TV state-sum with fixed boundary conditions. The embedding into $S^3$ part can only be done if we choose a spin structure on $S^1$ that can be smoothly extended to a spin structure on a disk, which means that we must choose anti-periodic boundary conditions for the $S^1$. Therefore, only half of the ground states on the collection of tori will come from tracing out id. The rest (on tori with periodic boundary conditions around the azimuthal cycle) come from tracing out some sort of twisted id functor, which has a $\pi$ flux inserted through the cycle of the $S^1$. 
