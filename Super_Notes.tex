...
we have the required anticommutation relations $\{\gamma_1,\gamma_2\} = 0$, as well as the identities $\gamma_1^2 = \gamma_2^2 = 1$. Since we have $c^\dagger c = i\gamma_1\gamma_2$ if $c = (\gamma_1 + i\gamma_2)/\sqrt{2}$, we have the intriguing relation 
...
Before beginning with computations, we recall some definitions and useful information regarding tubes and idempotents from earlier on in the notes. The multiplication table for non-vortex tubes  with non-zero charge (which is non-abelian) is \be
\renewcommand{\arraystretch}{3}
\begin{tabular}{c | c c c c r}
 $\times_A$          & $\FubeXss $ & $\FubeXsds $ & $\FubessX $&$ \FubessdX$  \\
\hline
$\FubeXss$ & $\FubeXss$ & $\ \FubeXsds$  & $\FubessX$ & $\FubessdX$  \\
...
\end{tabular}
\ee
Note that we have made the choice $A^4=i$ in the above table. For the vortex tubes with non-zero charge, the table (which is abelian) is 
\be
\renewcommand{\arraystretch}{3}
\begin{tabular}{c | c c c c r}
$\times_P$        & $\FubeXss $ & $\FubeXsds $ & $\FubessX $&$ \FubessdX$  \\
\hline
...
\end{tabular}
\ee
For the non-vortex spin structure, our idempotents are
\be m_\unit = \frac{1}{2}\left( \FubeXXX_A +\;\; \frac{1}{\sqrt{2}}\FubesXs_A\right),\qquad m_\psi = \frac{1}{2} \left( \FubeXXX_A -\;\; \frac{1}{\sqrt{2}} \FubesXs_A\right)\ee
and 
\be m_\sigma^\pm = \frac{1}{2} \left(\FubeXss_A \pm e^{i\pi/8}\ \FubessX_A\right).\ee

For the vortex spin structure, they are 
 \be q_\sigma = \FubeXXX_P\ee
 and 
 \be q_1 = \frac{1}{2}\left(\FubeXss_P + e^{-i\pi/8}\FubessX_P\right),\qquad   q_\psi = \frac{1}{2}\left(\FubeXss_P - e^{-i\pi/8}\FubessX_P\right).\ee
...
\be S^{sTr} = \frac{1}{\mcd} \begin{pmatrix} 
1&\sqrt{2}&1&0&0&0 \\
 \sqrt{2}&2\zeta^{-2}&\sqrt{2}&0&0&0 \\ 
 1&\sqrt{2}&1&0&0&0 \\ 
 0&0&0&2\zeta^2&2\sqrt{2}\zeta^2&-2\zeta^2 \\ 
 0&0&0&2\sqrt{2}\zeta^2&4&-2\sqrt{2}\zeta^2\\ 
 0&0&0&-2\zeta^2 & -2\sqrt{2}\zeta^2 & 2\zeta^2 \end{pmatrix}. \ee
This is very much not modular, but it still gives us some hints:
...
Finally, instead of tracing the $R$ matrices, we can appeal to a formula relating the $S$ matrix to the topological twists. In bosonic theories, we have 
...
\section{Notes on tensoring over endomorphisms}
We wish to describe a computational way to tensor over endomorphisms. 
Lets us first define what that means. 
We have two vector spaces $V^{ab}_x$ and $W^{xc}_d$. 
In our setup, these the vector spaces associated with trivalent vertices, and in the interesting case they will be tensored together by a line that can support odd endomorphisms. 
In that case the vector spaces are $\mathbb{Z}_2$ graded, and so are the endomorphisms. 
Choose a basis for $f_i \in \text{End}(x)$. 
The tensor product of the two vector spaces over $x$ is defined by,
\begin{align}
V^{ab}_x \otimes_{\text{End}(x)} W^{xc}_d \equiv (V^{ab}_x,W^{xc}_d ) /\prod_i \langle (V^{ab}_x f_i,W^{xc}_d ) \sim (V^{ab}_x,f_i W^{xc}_d ) \rangle.
\end{align}
That is the tensor product is a bilinear map on $V^{ab}_x \times W^{xc}_d$, taken modulo local relations. 
The local relations are all possible $f_i \in \text{End}(x)$.
This expression is rather formal and so it is useful to dissect it. 
The $f_i$ have a right action on $V^{ab}_x$ and a left action on $W^{xc}_d$. 
It is useful to encode this equivalence relation into a set of matrices $M(f)$ defined by,
\begin{align}
M(f). (V^{ab}_x,W^{xc}_d )  \equiv (V^{ab}_x f^{-1}, f W^{xc}_d )
\end{align}
So that vectors $X,Y  \in  (V^{ab}_x,W^{xc}_d )$ are identified if $M(f) X = Y$ for some $f \in \text{End}(x)$.
This action is complex linear as well $M(\alpha f_1 + \beta  f_2) = \alpha M(f_1) + \beta M(f_2)$. 
In fact these matrices form a finite matrix algebra, and in the interesting case they form a $\mathbb{Z}_2$ graded matrix algebra. 
Hence the vector space we associate to the tensor product is a module over this algebra and the physical states are the vector spaces left invariant under application of $M(f)$.
That is, vectors modulo these local relations form irreducible representations of $M(f)$.

Now lets reduce to the interesting cases. 
In the bosonic case we have $\text{End}(x)  = \mathbb{C}$ and so the usual tensor product holds. 
And the $M(f)$ is represented by $(v z^{-1},z w)$. 
Similarly for any M type particle. 
For a Q type particle we have $End(x) \cong \mathbb{C} \ell_1$ and so we have one even endomorphism and one odd. 
In this case the vector space becomes $\mathbb{Z}_2$ graded, but the matrices $M(f)$ all remain even. 
And therefore split into two blocks which preserve the parity of the states. 
There is always the identity map, then there is the one which accounts for the odd endomorphism which takes the even-even state to the odd-odd state, and similarly the even-odd state to the odd-even state. 
Let us call these matrices $E$ and $Y$. 

We can now consider fermionic $F$-symbols (the usual super-ones without dot slides) and impose the additional constraint on them that the matrices $E$ and $Y$ have to ``commute" with them. 
...
We can write these as matrices as well 
\begin{align} F^{abc}_d M(f_x) = M(f_y) F^{abc}_d. \end{align}
...
\section{Drinfeld center of half-E6}
Notation $e_{x}(\bullet ) = v^{x\bullet }_{\bullet x}$.These are in the ``idempotent" basis. (See below for conventional basis of Drinfeld Center.).
\begin{align}
e_{
       1       
}&=
...
Now we do it in conventional notation (although still complex conjugated from Wang's notation, i.e., object from category goes from bottom right to top left). 
\begin{align}
e_{
       1       
}&=
