...
\be \includegraphics{msig_def.pdf} \ee
 ...
 We can then calculate the braiding by moving the top punctures around eachother and re-expressing the result in terms of the basis vectors in $\fld^{b,a}_c$. For example, we can calculate the $R$-symbol $R^{m_\psi m_\sigma^+}_{m_\sigma^+} = -1$ as 
\be \includegraphics{mpsimsig_braiding.pdf}.\ee
When braiding $q$-type particles, we choose branch cuts that always meet in the center of the pants. For example, to calculate $R^{q_\psi q_{1/\psi}}_{m_{\psi/1}$, we write
\be \includegraphics{qpsiq1psi_braiding} \ee
where we have taken advantage of our knowledge of the eigenvalues of $q_\psi$ under the twisted tube. 

We define odd vectors in $\fld^{a,b}_c$ by placing dots on the top left leg of the pants. If this is impossible (e.g. if the top left leg is an $m_\psi$ idempotent), we put the dot on the top right leg of the pants. For example, for the odd fusion channel in $R^{q_\psi q_{1/\psi}}_{m_{\psi/1}$, we calculate
