...
In Figure~\ref{example_lattice} we illustrate the support of each operator appearing in the Hamiltonian with dashed circles, where we have drawn a section of the graph $\mcg$ embedded in the plane for the sake of visualization. 
...
If the edges are not oriented, then we would assign vertex Hilbert spaces as above, but with all edges pointing out of each vertex.
...
Note that the collection $\{\alpha(e)\}$ is determined by the spin structure of $\Sigma$ and
...
But for theories built out of unoriented strands, like $C_2$, it is convenient to allow this pathology.}
...
fusion spaces involving q-type objects, as discussed in Section \ref{fusion_spaces}. 
...
There is an obvious bijection between the boundary components of $\widehat\Sigma$ and the plaquette terms of the Hamiltonian.
...
Another way of achieving the same effect is to place a copy of the trivial tube category idempotent $e_0$
...
correspond to placing tube category idempotents at 2-cells~\cite{kirillov2011}.
...
Finally, we briefly comment on the spectrum of the Hamiltonian \eqref{ham} and the types of excitations it supports. 
