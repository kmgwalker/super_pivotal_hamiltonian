...
Diagrammatically, a vector in the image of $R_p$ takes on the form
\begin{align} 
...
we first insert a closed strand labeled $a$ into the interior of $p$, which results in the picture
\begin{align}
\Plaquetteb
\end{align}
Next, we begin fusing the strand into the plaquette using the resolution of the identity \eqref{identity}, which gives the picture
\begin{align}
\label{plaquette_first_F_move}
\Plaquettec
\end{align}
Next we apply a sequence of $F$-moves to the diagram, the first of which gives
\begin{align}
\Plaquetted.
\end{align} 
We repeat this procedure until we are left with a diagram of the form
\begin{align}
\Plaquettee =\\ 
=\Plaquettef
\end{align}
The bubble part of this diagram (the 2-cell bounded by the edges $x_6$ and $a$) can 
be removed at the expense of a theta symbol, which cancels the theta symbol 
introduced in \eqref{plaquette_first_F_move}.
...
and so we may apply them in any order we wish. Indeed, an equally tedious way to 
