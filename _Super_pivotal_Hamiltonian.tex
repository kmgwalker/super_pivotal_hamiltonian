...
\end{align}
where the sums are over the plaquettes (faces) $\mcf$, edges $\mce$, and vertices $\mcv$ of the graph, %\ethan{we used mathcal stuff in the state-sum section, so I mathcalized the things to be consistent}
...
\Plaquettea
\end{align}
We will write down the matrix elements of $B_p^a$ in this basis, which we call the {\em standardized} basis.
...
\ethan{you're a hero for doing the above. I think we should mention $B_p$ being the trivial 
idempotent in $\tube$ though, probably by deleting the next paragraph and keeping (with edits) the one after that.}
\dave{Yea, definitely.}
...
weighted superposition of all closed loops labeled by simple objects in the input super fusion 
category $\mcc$.
