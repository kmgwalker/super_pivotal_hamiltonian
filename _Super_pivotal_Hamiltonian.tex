...
to their length. Alternatively, one may 
modify the Hamiltonian to be defined on a graph with punctures inheriting non-bounding 
spin structures at the locations of the vortex particles, and to incorporate the vortices effect on the spin structure into the $\alpha(e)$ factors appearing in the expression for $D_e$. 
Doing this gives the spin structure defect lines zero energy; they become tensionless and are free to fluctuate under the action of flat $\zt$ gauge transformations acting on $C^1(\mcg^*)$. 
The vortices are still not truly deconfined however, since moving them without changing the 
