...
\kw{We need to somehow make it clear that incoming/outgoing is independent of the orientation of the edges incident to the vertex.}
...
\kw{Would it be better to say that $\Gamma_i$ is only defined with the $i$-th slot is q-type?}
\dave{I would prefer that. I made the change.}
...
\dave{
We could reduce it to a footnote, 
something like: \footnote{The matrix elements of $\Gamma_1$ can be written as $[\Gamma_1]_{\psi \eta}  ={\BananaDot} \left(  \Bananaeta \right)^{-1} $.
Similarly for $\Gamma_2$ and $\Gamma_3$. }}
...
\begin{align}
H = \lambda_p \sum_{p \in \mcf} (1-B_p)  + \lambda_e \sum_{e \in \mce} (1-D_e) + \lambda_v \sum_{v \in \mcv} (1-A_v)
\end{align}
...
In Subsection \ref{modified_tensor_product} we solved the same problem in
...
%%%%
\dave{proposed re-write:}
In general, if $e$ is oriented from $v_1$ to $v_2$ colored by a fixed object $x$ and $|\psi_1\rangle \tp |\psi_2\rangle$ is a vector in $\mch_{v_1}\tp \mch_{v_2}$, the edge term
can be written as the projector
\begin{align}
D_e = \frac{1}{2}(1+J_e)
\end{align}
with $J_e$ implementing the local relations coming from $\End(x)$:
...
B_p =\frac{1}{\mcd^2} \sum_{a \in \mcc} \frac{d_a}{\text{dim} \; \text{End}(a)} B^a_p, 
\end{align}
...
\end{align}
with each $P_j = \bigoplus_{abc} (P^{abc} )^{l_j}$ defined in \eqref{Pitchfork_pivot} and with $l_j = 0,1,2$ controlling the angle by which the $j$th vertex is pivoted (if the edges aren't all oriented out of the vertex then the appropriate object in $P^{abc}$ needs to be replaced by its dual).
...
&\times \Plaquetteg
\end{align}
The pairing guarenties that $\sigma_6$ is dual to $\sigma_0^*$ and the fraction cancells the intial one introduced in \eqref{plaquette_first_F_move}.
Noting that $|\sigma_6| = |\sigma_0|$ we have,
\begin{align}
\nonumber
...
The plaquette operator, \eqref{plaquettematrix} represents the action of inserting an $\omega$ loop into the center of the plaquette and fusing it into the neighboring edges and vertices.
%acts to insert a 
...
Equivalently, $B_p$ implements the action of the trivial idempotent in the tube algebra applied to the plaquette $p$:
\begin{align}
B_{p} = e_{0}(p).
\end{align}
...
The Hilbert space it acts on is given by all nets modulo local relations on $\Sigma \setminus \text{2-cells}$ (i.e., we remove a disk from every 2-cell). 
Each disk residing in the 2-cells have atleast one marked point. 
If the marked point has any label other than the trivial label $B_p$ annhiliates the state.
Excitations correspond to states that $B_p$ annihiliates, and are in 1-to-1 correspondence with the minimal idempotents of $\tube(\mcc)$.
...
The deconfined anyonic excitations in the model correspond to violations of the plaquette terms,\footnote{Note that from the structure of the Hamiltonian, pure flux excitations violate only plaquette terms, while general dyonic excitations (with nonzero charge) violate both plaquette and vertex terms.} 
and by construction, these are in one-to-one correspondence with the bounding idempotents of the tube algebra. 
