...
%$\omega_{\mcc} =(\cl(\unit) +\cl(\psi))\cdot \omega_{\mcc/\psi}$, 
%and that $\cl(\psi)$ can be removed at the compensation of the phase $s(J) (-1)^{ \nu_y}$.
...
%\kw{I changed indices here; Dave should double-check}
%\dave{I changed $V^{x}_{xc} \ra V^{x'}_{xc}$}
...
%If $\alpha = \psi$ then we require $y \tp \psi \cong y'$.
...
%\end{align}
%\kw{Maybe make the boundary circles of $P$ a different color so that they are not confused with ribbons.}
%\dave{No problem. 
%Not sure what color looks best, I tried green, looks kind of funny. 
%What if we left the circles of $P$ black, and colored the $r,st,u,v,$ green, and $\omega$ loops red (could do same for the analogous diagram above). }
%\kw{Also, I think you forgot to drop the outer omega on the RHS.}
%\dave{I think I left it there intetiontionally, but I forget why and agree it's not necessary.
%I removed it.}
%\dave{Colored internal lines red and omega loops green.}
...
%In the last equality, we have introduced $\tilde{\omega} =\frac{1}{\mcd^2}\sum_x \kappa_x \cl(x)$, 
...
%As with \eqref{...} this is accomplished by simultaneously letting $\cl(y) \ra \cl((y + y\tp\psi)/2^{n_x}$
\begin{align}
...
%\begin{align}
%\cl_W(f^{J}_{ab}) \xrightarrow{S^{JW \ra WJ}}  \sum_{\substack{x \in \sob(\mcc) \\ y \in \sob(\mcc/\psi) }} S_{ax}  S_{by}^*   \cl_J(f^{W}_{xy})
%\end{align}
%\begin{align}
%\cl_W(E(a,b)) \xrightarrow{S^{JW \ra WJ}}  \sum_{\substack{x \in \sob(\mcc) \\ y \in \sob(\mcc/\psi) }} S_{ax}  S_{by}^*   \cl_J(E(x,y))
%\end{align}
\begin{align}
...
\end{align}
%Recall that $\cl_W(\widehat{E}(a,b))$ means 
...
\ethan{If you guys are uncomfortable with writing the twists like that because of spin structure permutation issues (I forget if we don't like it or not) I can do something else or expand on what the twists really mean.}
\dave{I think it's probably OK, we're just plugging in numbers at this point.}

%\kw{Have we carefully compared the formulas of this section to the C2 S and T matrices?
%If not, I think we should do so.}
%\dave{Agreed.
%I'll try to do this in the very near future.}
%\dave{Will do this, or ask Ethan to.}
%\ethan{I did it, and it works, see above.  Can also write down the $S$-matrix verification, but it's a bit tedious and un-illuminating.}
%\dave{Just to be sure, you also checked that the S-matrices were the same?
%Thanks a lot!} \ethan{jah}
