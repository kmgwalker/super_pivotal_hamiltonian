...
framework needed to formally develop classification efforts \cite{kitaev2006}\dave{Should add a few other references.}. 
...
%\davex{Should probably get the historical order correct here. 
...
%\ethan{what about this:}
%\kw{I googled for ``local particle" and did not come across that term being used in this sense (though perhaps I missed something).
%I think we should avoid coining new terminology for old concepts.}
...
%\davex{I think the locality part is confusing. 
%What about: ``a cousin to bose condensation, where pairs of physical and emergent (anyonic) fermions are condensed."
%Not totally happy with that sentence either.
%The reason being the ambiguity in ``pairs of physical and emergent (anyonic) fermions" could mean we condense a ``$(\psi f) (\psi f)$ composite" or a just a ``$(\psi f)$ composite".
%(Physically, I think we are doing something akin to $(\psi f) (\psi f)$, 
%while mathematically we are condensing $(\psi f)$.) 
%}
%\davex{This is a nice sentence. But subtle. 
...
%\ethanx{Right. I'm saying it means you can create/destroy something in a condensate locally. Feel free to add whatever explanatory remarks you like}
%\kw{We don't talk about phase transitions in the body of the paper, so perhaps it does not make sense to mention them here}
%\davex{True, and I would prefer to keep it that way (meaning no discussion of phase transitions in the body of the paper). 
...
%\davex{I think the vote is to remove any mention of phase transition.}
%\davex{
...
%Physical perturbation is not free.
%}
There have been several recent approaches related to understanding 
fermionic topological phases with field theoretic methods, fermion condensation, and bosonization \cite{Barkeshli2014,barkeshli2015,gaiotto2016, bhardwaj2016, bhardwaj2016b,kapustin2017,putrov2016}, 
with a more algebraic take on fermion condensation given in \cite{wan2016}. 
\dave{I lumped Maissam's papers \cite{Barkeshli2014,barkeshli2015} in the list above.
I think they (roughly) look at how electrons can be transmitted/absorbed as emergent fermions in various QH and QSH platforms.
Though I didn't read much beyond the abstract and looking at figures. }
...
%\davex{Maybe we should say heuristically rather than physically (physically I think we are condensing $(\psi f) (\psi f)$.)
%}
...
%\kwx{to do: make sure ``anyon" etc is used consistently in intro (perhaps this has already been addressed}
...
%\davex{Lets remove the last sentence.}
...
%\ethan{how about the following? This way we avoid saying super interchange law in the introduction (since we don't say it again in the paper)\davex{Sounds good to me.}} 
...
%\davex{Should we mention Drinfeld center here? I thought there was some things to flesh out (also we only mention bosonic Drinfeld centers in the paper)
%We could replace Drinfeld Center with Turaev Viro TQFT, or something akin to that.}
...
%\ethanx{may want to avoid reference to Drinfeld centers}
%\kw{I agree: drop last sentence}
...
%\kw{Proposed addition; not essential}
%\ethan{I like it}
%\davex{Me as well.}
...
and study the modular transformations %and braiding data 
of the theory. 
...
$\tube(\mcc/\psi)$. 
%\davex{Could say ``We also compute the modular data for $\tube(\mcc/\psi)$."}
%\ethan{I dig it}
...
%how the fermionic nature of the vector spaces used in the construction of super 
%pivotal categories affects their construction. 
%\kw{last sentence sounds odd to me.  can we be more specific?  various, change to }
...
%and the violation of which produces linearly confined defects. 
%\kw{deleted a reference to spin defects here, since we already mentioned it earlier in the intro}
...
%\davex{Do we have a name for a generic net configuration, 
...
\end{longtabu}
%\davex{Maybe add blue line labeled fermion, orange line for q-type and black for all around?}
%\ethan{added:}
%\kw{I think there are places where we use orange (or at least orangish) strands for non-q-type objects (e.g.\ in the review of the E6 theory at the start
%of section 7)}
%\dave{True. We could just not mention it, or fix the figures. Both are easy.}
%\kw{I lean toward not making the promise that we are going to follow this rule with colors}
%\dave{Sounds good to me. Should we remove/modify the following sentence then:}
%When drawing diagrams, blue lines and dots will always indicate fermions, orange lines will indicate q-type objects, 
%and black lines will be used for both m-type and q-type objects. 
%\davex{Define notation $\cc^{p|q}$.}
%\ethan{added to the table}
