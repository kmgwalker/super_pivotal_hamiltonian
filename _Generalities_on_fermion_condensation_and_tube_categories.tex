...
\end{cases}
\end{align}
or in a more more suggestive notation: $\langle \text{cl}_B(x), \text{cl}_B(x) \rangle = \dim \End(x)$.
The total dimension $\mcd^2$ is given by $ \langle S^2_\phi, S^2_\phi \rangle$, the inner product of the 2-sphere (empty boundary conditions) with itself.
...
\end{align}
The 3-ball with a single closed up $x$ strand has partition function $Z(D^3, \overline{\text{cl}(x)}) = d_x$. 
Hence the total dimension is given by,
\begin{align}
