...
but I think we should probably choose a different charector for that as well (maybe $\mathcal{Q}$).
}
...
%\dave{I agree.}
%\dave{I'm confused by this, I thought there was no Koszul sign. 
%e.g., what if $f=g = \gamma$, an odd endomorphism. 
%Then $\tr(fg) = -\tr(gf)$ but $fg = gf \propto \text{id}$.
%}
%%KW You guys are right; I was being stupid
\be  \label{trace_cycle_rel}
%	\tr(fg) = (-1)^{|f||g|} \tr(gf) ,
	\tr(fg) = \tr(gf) ,
\ee
%\ethan{can you explain the signs here? It seemed like we got $(-1)^{|f|}$ from rotating $f$ around, and then a koszul sign from interchanging the koszul order of $f$ and $g$. Since $\tr$ is even, $|f|=|g|$ if $\tr(gf)\neq0$, and we get $\tr(fg) = \tr(gf)$. Plus as written, we get $\tr(f^2)=0$ for any odd $f$ (like a dotted $\beta$ strand), which doesn't seem correct}
...
\be
	\langle fg, h \rangle =(-1)^{|g||h|} \langle f, h \bar{g} \rangle \;\;\; {\rm and} \;\;\; \langle fg, h \rangle = (-1)^{|f||g|}\langle g,\bar{f}  h  \rangle 
\ee
\kw{I think the one of the two Koszul signs should be dropped.  Do you agree?\dave{I keep getting two Koszul signs (see below)}}
\kw{Also, the second K sign was originally fh instead of fg (I changed it yesterday)\dave{I'm still getting $fh$ (calculation below)}}
...
\dave{proposed addition: Note that we allow for $x \cong a_1 \tp a_2 \tp \cdots a_k$ so that a basis of $\mor(\unit \to x)$ could be relatively large.
However, often $x$ will denote a simple object in and so $\mor(\unit \tp x)$ is one dimensional: 
\begin{align}
\label{Disk_Hilb}
...
\kw{shouldn't we remark that this vectos space is zero unless $x \cong \unit$? (assuming $x$ is simple)}
\dave{I added a note. 
Also, when I wrote this I was allowing for $x \cong a_1 \tp a_2 \tp \cdots$, and hence the $\mor(\unit \to x)$.
Is that legitimate? 
...
\dave{Yes. Will do the norm of q-type = 2 here. Roughly:}
Let $x \in \sob(\mcc)$. 
Denote $\text{cl}_B(x)$ the closure of $x$ on annulus with bounding spin structure:
\begin{align}
\text{cl}_B(x) = \text{picture}
\end{align}
Then,
\begin{align}
...
\end{cases}
\end{align}
\dave{Do this for $\text{cl}_N(x)$ to show m-types have to be closed evenly, and q-types oddly.}
