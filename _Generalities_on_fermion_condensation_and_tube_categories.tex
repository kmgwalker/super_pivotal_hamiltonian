...
described in this section that will also be of use to us later.
The first is to compute the total dimension of the category.
...
Denote $\text{cl}_B(x)$ the closure of $x \in \sob(\scat)$ on an annulus with bounding spin structure:
\begin{align}
...
The axioms of TQFT imply
\begin{align}
\langle S^2_\phi, S^2_\phi \rangle = Z(S^2 \times I)(S^2_\phi \cup \overline{S^2_\phi}) = \sum_{x \in \sob(\scat)} \frac{Z(D^3, \text{cl}(x)) Z(D^3, \overline{\text{cl}(x)}) }{\langle  \text{cl}_B(x), \text{cl}_B(x) \rangle},
\end{align}
where $D^3$ is the 3-ball. 
A 3-ball containing a single closed up $x$ strand has partition function $Z(D^3, \overline{\text{cl}(x)}) = d_x$ by definition of the quantum dimension. 
Hence the total squared dimension is given by
\begin{align}
