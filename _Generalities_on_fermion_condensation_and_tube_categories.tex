...
need to think about where to first introduce $\spc$}
\ethan{Actually I think right here is a suitable place. Maybe something like the following few sentences could work:}
...
with $\spc$ a super pivotal category obtained through a fermionic quotient ($\mcc/\psi$) or more 
...
\dave{I will do this. (make note for colabs to read it)
...
\ethan{we did, now it's introduced where we define the various tube categories}
...
of a 3-ball with a closed $x$ loop on its surface (again, $\scat$ is an arbitrary super linear category). 
\kw{I don't think $\scat$ is appropriate here; let's discuss.
In general I think we should use $\scat$ only when there is no need for a tensor/monoidal structure anywhere in the story.}
...
\begin{align}
\mcd^2 = \sum_{x\in \sob(\scat)} \frac{d_x^2}{\dim \End(x)}.
\end{align} 
Note that letting $ \scat = \tube(\spc)$ gives the total dimension of tube category.
...
%The tube category
Using \eqref{DC_and_DCpsi} again and specializing to the case where $\mcc$ is a modular tensor category then $\tube(\mcc/\psi) \cong \mcc \times (\mcc/\psi) $ 
...
\dave{I'm enthusiastically in favour.
...
\ee
A special case of \eqref{amordef} yields
\be  \label{coldotprod}
	\mor(x_i\to x_j) \cong \bigoplus_\alpha \Hom(W_{i\alpha} \to W_{j\alpha})\otimes_\cc \End(e_\alpha) .
\ee
When $i\ne j$, this is merely an isomorphism of super vector spaces, but when $i = j$ this is an isomorphism of super algebras.
...
See Section \ref{he6mp-tube-cat} for an example of this.
