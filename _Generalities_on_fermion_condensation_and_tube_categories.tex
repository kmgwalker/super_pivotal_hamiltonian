...
We can then compute
\begin{align}
\langle S^2_\phi, S^2_\phi \rangle = Z(S^2 \times I)(\overline{S^2_\phi} \cup {S^2_\phi}) = \sum_{x \in \sob(\scat)} \frac{Z(D^3, \cl(x)) Z(D^3, \overline{\cl(x)}) }{\langle  \cl_B(x), \cl_B(x) \rangle},
\end{align}
where $D^3$ is the 3-ball and $Z(D^3,\cl(x))$ denotes the partition function 
...
We can compute $Z(D^3, {\cl(x)}) = Z(D^3, \overline{\cl(x)}) = d_x$ by definition of the quantum dimension, and hence the total squared dimension is given by
\begin{align}
...
Similarly, one can show that $\langle \cl_N(x\cdot \gamma), \cl_N(x\cdot \gamma) \rangle$ 
...
The norm squared of $\cl_W(x)$ is
\begin{align}
\langle \cl_W(x), \cl_W(x) \rangle = Z(T^2 \times I)(\overline{\cl_W(x)} \cup \cl_W(x)) = Z((S^1 \times I)\times S^1_W)(\overline{\cl_W(x)} \cup \cl_W(x)).
\end{align}
...
As in \eqref{clxnorm}, 
the $W$ dependent matrix trace will be $\dim \End(x)$ (if $\cl_W(x)$ is nonzero) and so 
\begin{align}
\langle \cl_W(x), \cl_W(x) \rangle = \dim \End(x)
\end{align}
