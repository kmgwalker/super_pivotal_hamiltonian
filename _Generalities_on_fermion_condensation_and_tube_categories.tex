...
Figure \kw{\ref{ExampleMorphismsTubeC}; includes annuli with multiple endpoints and non-matching numbers of endpoints}
\dave{Added a figure.} shows some examples
...
\end{align}
\caption{\label{ExampleMorphismsTubeC}Some examples of morphisms in $\tube(\mcc)$.
We have written the annulus as a square with left and right sides identified. 
At the bottom left each morphism denoted the spin structure B(N) for bounding(non-bounding). 
The labels $a,b,c,d \in \sob(\mcc/\psi)$, and $\nu \in \mor_{\mcc/\psi}(d \tp b \ra c)$, and $\mu \in \mor_{\mcc/\psi}(a \ra b \tp d)$.
}
\end{centering} 
\end{figure} 
...
In what follows $\spc$ is a super pivotal fusion category coming from a fermionic quotient, 
or satisfying the assumptions of Section \ref{def_sect}.
...
Often $x$ will denote a simple object (every net configuration on the annulus is isomorphic to one of this form) and so $\mor(\unit \to x)$ will be one dimensional: 
\begin{align}
\label{Disk_Hilb}
...
described in this section, which will also be of use to us later.
First, we will compute the total dimension of the category $\scat$.
An important intermediary step is to compute the norm squared of a single strand on an annulus with empty boundary conditions.
We denote by $\text{cl}_B(x)$ the closure of $x \in \sob(\scat)$ on an annulus with bounding spin structure:
\begin{align}
\text{cl}_B(x) \; = \; \underset{B}{\clx}
\end{align}
The the norm squared of $\text{cl}_B(x)$ is
\begin{align}
...
Since $\mor(\unit\ra x\tp x^*) \cong \mor(x\ra x) \cong \End(x)$, we get
 \begin{align}
 \label{clxnorm}
 \langle \text{cl}_B(x), \text{cl}_B(x) \rangle = \dim \End(x) = 
\begin{cases} 
1 & \text{if $x$ is m-type} \\
2 & \text{if $x$ is q-type} 
\end{cases}.
\end{align}
The total squared dimension $\mcd^2$ of the theory is given by $ \langle S^2_\phi, S^2_\phi \rangle$, the inner product of the 2-sphere (with empty boundary conditions) with itself.
We can then compute
\begin{align}
\langle S^2_\phi, S^2_\phi \rangle = Z(S^2 \times I)(\overline{S^2_\phi} \cup {S^2_\phi}) = \sum_{x \in \sob(\scat)} \frac{Z(D^3, \text{cl}(x)) Z(D^3, \overline{\text{cl}(x)}) }{\langle  \text{cl}_B(x), \text{cl}_B(x) \rangle},
\end{align}
where $D^3$ is the 3-ball and $Z(D^3,\text{cl}(x))$ denotes the partition function 
of a 3-ball with a closed $x$ loop on its surface. 
To obtain the second equality, we have written $S^2\times I$ 
as a union of two manifolds homeomorphic to $D^3$ glued along a bounding annulus and 
have made use of the gluing axioms of TQFT. 
We can compute $Z(D^3, {\text{cl}(x)}) = Z(D^3, \overline{\text{cl}(x)}) = d_x$ by definition of the quantum dimension, and hence the total squared dimension is given by
\begin{align}
...
where we pointed out that $\text{cl}_N(\beta)$ is identically zero.
...
We now show that closing up a q-type idempotent onto a torus results in a state with norm $\sqrt{2}$.
Let $x$ be a minimal idempotent of $\tube(\spc)$, 
and $\text{cl}_W(x) \in A(T^2)$ be the string net found by closing $x$ onto a torus with spin structure $W$ 
along the newly closed cycle. 
For the moment, assume the newly formed torus is bounding.
The norm squared of $\text{cl}_W(x)$ is
\begin{align}
\langle \text{cl}_W(x), \text{cl}_W(x) \rangle = Z(T^2 \times I)(\overline{\text{cl}_W(x)} \cup \text{cl}_W(x)) = Z((S^1 \times I)\times S^1_W)(\overline{\text{cl}_W(x)} \cup \text{cl}_W(x)).
\end{align}
...
The the linear map we need to take the matrix (super) trace of is found by cutting open the cycle $S^1_W$. 
...
As in \eqref{clxnorm}, 
the $W$ dependent matrix trace will be $\dim \End(x)$ (if $\text{cl}_W(x)$ is nonzero) and so 
\begin{align}
...
Namely, if $t \in \tube^W(\spc)$ is an endomorphism then $\tr(t) = Z(S^1_W \times S^1_B)(\text{cl}_B(t))$. 
