...
where as usual $A(Y)$ denotes string-nets modulo local relations on $Y$ \ethan{slightly misleading since we usually write $A(Y)$ but now we're specifying a string-net boundary condition}.
...
\be \horizbeta \tp_\cc \horizbetadot \sim \horizbetadot \tp_\cc \horizbeta.\ee 
...
discussed above, and in this case are given by the structure constants of $\cliffiff_1$ \cite{turzillo2016}. 
