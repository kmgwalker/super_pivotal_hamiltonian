...
In what follows, 
we will show that a single strand of $\beta$ string is a diagrammatic description for the zero correlation length limit of the Kitaev chain\dave{I think Kapustin/Thorngren have a paper that mention something along these lines should probably cite them.}.
...
\dave{Did we use $V$ for any particular reason, or should it be $\mca$.} 
\ethan{I don't know if it ``should'' be anything, it's just a choice}
\dave{$\mca$ seems more natural to me so lets go with that,
 we can always change back to $V$.}
\be\label{VIbetabeta}
 \mca(I;\beta,\beta) = \cc \left[ \halfchain\;, \; \halfchaindot \right] = \cc^{1|1}.\ee
 
 Now we can consider splitting the interval $I$ into two smaller intervals $I_1,I_2$, such that $I_1\cup I_2 = I$.
 We then can reconstruct the vector space $\mca(I;\beta,\beta)$ from the vector 
 spaces $\mca(I_1;\beta,\beta),\mca(I_2;\beta,\beta)$ by gluing the two intervals $I_1,I_2$ together. 
...
The standard tensor product $\tp_\cc$ doesn't work, since then $\mca(I_1;\beta,\beta) \tp_\cc \mca(I_2;\beta,\beta) \cong \cliff_2 \not\cong \mca(I;\beta,\beta)$. 
...
%We thus can write 
\be \mca(I;\beta,\beta) \cong \mca(I_1;\beta,\beta) \tp_{\End(\beta)} \mca(I_2;\beta,\beta),\ee
...
\halfchain \tp_\cc \halfchaindot \; &=\; \halfchaindot \tp_\cc \halfchain\; \quad \quad \text{and,}\\
\nonumber
...
\begin{align}
\mca(I_i; \beta, \beta) = \cc \left[ \LocalHilba, \LocalHilbb \right].
\end{align}
...
\begin{align} 
\label{HilbInterval}
\mch_I  = \mca(I_1; \beta, \beta)  \tp_\cc \mca(I_2; \beta, \beta)  \tp_\cc \cdots \tp_\cc \mca(I_N; \beta, \beta).  
\end{align} 
...
&\;\; \vdots
\end{align}
Since we used $\tp_\cc$, we have too many states; we have not yet modded out by the equivalence relations \eqref{graphical_equiv_reln}
...
\end{align}
which are non-trivial only on the adjacent Hilbert spaces associated to the intervals $I_i$ and $I_{i+1}$.
...
\end{align}
This Hamiltonian describes the zero correlation length limit of the Kitaev chain, albeit in a slightly obscurred language.
To merger this gap, we now investigate the ground state wave functions.
...
\dave{Agreed. I re-phrased a little, let me know if it's to your liking.}
...
Indeed, the nontrivial term in the Hamiltonian is proportional to the fermion parity measured between adjacent physical sites. 
Acting with that term on-site, we find
...
\end{align}
which is precisely the action of $(-1)^F$, and can be identified with $i\gamma_1\gamma_2$ in the conventional language.
\dave{Should check this. It's not immediately obvious that $(-1)^F$ can be identified with $i \gamma_1 \gamma_2$.
Could also re-word to circumvent that.}
...
A nice feature manifest to the diagrammatic notation is that the local fermion number is not a good quantum number.
...
To better understand the wavefunctions $\Psi_e,\Psi_o$, we can apply ana series of F-move's to change back to our physical ``on-site" basis. \footnote{Recall the physical Hilbert space is asscociated to $\mca(I_i,\beta, \beta)$, 
...
\dave{Ethan, I found no minus sign in front of $A^4$ -- probably since the local basis for $v_e$, $v_0$ has a squiggle in it now, 
so that the fermion is effectively on the oppisote side of the cups. 
Can you double check that I got this correct? (I also found $n_f-1$ in the exponent of the odd wavefunction.}
...
We also remark that the simplicity of these wavefunctions is a result of only considering the $C_2$ theory.
...
Note that the fermion dot appearing in $\Psi_o$ of \eqref{kitaev_wire_ground_states} has zero energy, while this is not true for fermion dots 
...
By considering the same spin chain but using the Ising fusion category $A_3$, rather than $A_3/\psi$ one finds exactly the transverse field Ising model. 
...
Of course, we have only discussed the zero correlation length limit, but on-site terms can be added as well, and one finds the similar results, zero modes de-localize, etc.  
