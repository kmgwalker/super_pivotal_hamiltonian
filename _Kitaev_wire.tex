...
\dave{I tried to continue this trend.}
We seek an MPO that projects a given wavefunction into the image of the projectors $H_i$ defined in \eqref{kitaev_chain_ham_sitei}. 
...
Once expanding the product $\prod_i H_i$ we find a sum overall all possible configurations of the operators $e_i$ or $f_i$ straddling the junctions between intervals $I_i$ and $I_{i+1}$. 
Denote the resulting wavefunction $\Psi$.
...
Hence, when we simplify $\Psi$ the phase factor associated with a given physical site $v_i$ will depend on a pair of indices $(e,e)$,$(e,f)$,$(f,e)$, and $(f,f)$.
The left(right) index denotes which term in $H_{i-1}(H_i)$ contributed to the phase.
...
\dave{changed $A$ to $W$ in the following.}
...
For example, the diagrammatics allow one to check that $W_{ef}$ is an odd operator and by construction satisfies $W_{ef} = A^4 W_{fe} \circ (-1)^F$.\dave{Need to double check this.}
...
To obtain an MPS for the ground states we simply fix an initial vector and boundary conditions for the MPO. 
...
\end{align} 
Using $W_{ff}$ in both cases would provide the same wavefunction up to an overall phase.
