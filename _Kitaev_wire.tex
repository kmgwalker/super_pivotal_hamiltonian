...
%The projector that mods out by the equivalence relations \eqref{graphical_equiv_reln}, 
%is given by 
\begin{align} 
...
\dave{This should be in the conclusions of this subsection.}
This also completes the connection between the Hamiltonian defined in Section \ref{Super_pivotal_Hamiltonian} based on the $C_2$ theory the previous works of \cite{tarantino2016,ware2016}.
We can also view it as the completely staggared limit of an anyon spin chain (which can be defined for any fusion category) \cite{Feiguin2007}.
In the language of anyon spin chains, we could add an on-site coupling 

%We make a few remarks about how this sits with other known results.
%First, we note that since each $\beta$ strand in the $C_2$ theory can be thought of as a Kitaev wire, 
%we've made a connection between the Hamiltonian in Section \ref{Super_pivotal_Hamiltonian} and the ones previously studied in  \cite{tarantino2016,ware2016}.
%Second, we note that in this 1D setting, we can make a rather explicit connection between fermion condensation, 
%and the Jordan-Wigner transformation. 



for the $C_2$ string net defined in Section \ref{Super_pivotal_Hamiltonian}

%As the notation suggests, $H$ is the Hamiltonian for the Kitaev wire. 
%In what follows, we will see how this comes about from a slightly more concrete point of view. 

\begin{comment}
%some unused equations 
%Modding out by these equivalence relations in Equation \eqref{graphical_equiv_reln} can also be done by taking the outer tensor product of $V(I, \beta, \beta)$ but require that the vectors are invaraint under a particular idempotent 

%\dave{had to go, will come back to this later this afternoon.}
%%%%%%
%%%%%%
%%%%%%
%%%%%%
...
Graphically, utilizing $\tp_{\cliff_1}$ is equivalent to modding out by the equivalence relations
\be
...
\ethan{I still think the following picture and the one after are helpful for deriving what the ground state wfs look like, and that they offer a complimentary perspective to the `cut and glue' approach above. }
\dave{I find the starting point a little confusing. Are we talking about anyon spin chains or 1+1TQFT's?
Can you explain to me what the point punch line is?
I think some of the content should be kept, e.g., talking about $M = S^1_B , S^1_N$, but the stuff about writing the wavefunction using the TQFT should either be cleaned up or removed. 
}
...
Vectors in $\mch(I)$ can then be written as $a_1\tp\dots\tp a_N$ where $a_i \in \{\unit,\beta\}$\dave{I don't see how this is a vector in $\mch(I)$.
Maybe in $\mca(D^2,a_1 \tp \cdots \tp a_n)$ or something like that. 
But then it doesn't define the vector uniquely.}, and graphically 
...
\dave{I'm a little confused what you're going for here. 
But if it's an interval living at the boundary of a string net then it would be natural to have strings that come off of it into the bulk of the string net, and also a boundary condition at the ends of the interval.
If it's an anyon spin chain on an interval, it's also natural to have the boundary conditions (and distinguish them from $a_1$ and $a_N$ which look like ``physical" dof.). 
}
...
In the language of our earlier discussion, each tail $a_i$ corresponds to the end of a strand $I_i$ in the 
decomposition \eqref{HilbInterval}. 
...
\dave{I don't follow the previous two paragraphs.}
