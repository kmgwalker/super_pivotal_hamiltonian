...
These are exactly the properties of a Majorana zero mode.%, and in this case a Majorana zero mode.
...
Fermion condensation provides a map a map between these two models in the same way the Jordan-Wigner transformation does\dave{could add: `; also noted by \cite{turzillo2016,kapustin2017}', not sure if we need to cite people for this. }.
...
Using this, the fact that the structure of the Hamiltonian is the same as in \eqref{kitaev_chain_ham_sitei}, and using our earlier results \eqref{kitaev_wire_ground_states}, we can write the wavefunctions on the $B$ and $N$ sectors as 
...
\dave{I think we can change the notation a bit.}
We need to write an MPO that lies in the image of the projectors $H_i$ defined in \eqref{kitaev_chain_ham_sitei}. 
...
\dave{Should rename the $A$ tensors here since it is already reserved in the $C_2$ theory.}
...
We thus obtain an MPO,
\begin{align}
A_{b_l b_r}: \; \mch_I &\to \mch_I\\
v_1 \tp \cdots \tp v_n& \mapsto  \sum_{\{v_i' \}} \left( A^{v_1 \ra v_1'} A^{v_2 \ra v_2'} \cdots A^{v_n \ra v_n'} \right)_{b_l b_r} \; v_1' \tp \cdots \tp v_n' 
\end{align}
where $b_l$ and $b_r$ are boundary conditions taking labels in $\{ e, ,f \}$.
They denote whether an additional fermionic dot has been added to the leftside of the first interval or the rightside of the last interval. 
For example, the diagrammatics allow one to check that $A_{ef}$ is an odd operator and by construction satisfies $A_{ef} = A^4 A_{fe} (-1)^F$.\dave{Need to re-label all the A-tensors.}
...
To obtain an MPS for the ground states we simply fix an initial vector and boundary conditinos for the MPO. 
On the interval we can construct the even parity ground state by
\begin{align}
\Psi_{I,e} = \sum_{\{v_i' \}} \left( A^{v_e \ra v_1'} A^{v_e \ra v_2'} \cdots A^{v_e \ra v_n'} \right)_{ee} \; v_1' \tp \cdots \tp v_n' 
\end{align} 
and the odd parity ground state with
\begin{align}
\Psi_{I,o} = \sum_{\{v_i' \}} \left( A^{v_o \ra v_1'} A^{v_e \ra v_2'} \cdots A^{v_e \ra v_n'} \right)_{ee} \; v_1' \tp \cdots \tp v_n'.
\end{align} 
Using $A_{ff}$ in both cases would provide the same wavefunction up to an overall phase.
Similarly using \eqref{bounding_trace}, one can find the MPS on a bounding spin circle by,
\begin{align} 
\Psi_B = \sum_{\{ v_i ' \} } \tr \left( A^{v_e \ra v_1'} A^{v_e \ra v_2'} \cdots A^{v_e \ra v_n'} \right) v_1' \tp \cdots \tp v_n' 
\end{align}
and using \eqref{nonbounding_trace} we find the MPS on the non-bounding circle,
\dave{need to double check these.}
\begin{align} 
\Psi_N = \sum_{\{ v_i ' \} } \str \left( A^{v_o \ra v_1'} A^{v_e \ra v_2'} \cdots A^{v_e \ra v_n'} \right) v_1' \tp \cdots \tp v_n'.
\end{align}
In order to have a non-zero wavefunctions we needed to choose a parity even initial vector for the bounding spin structure, 
...
One can think of the string net Hamiltonian (defined in Section~\ref{def_sect}) for the $C_2$ theory as describing a phase of fluctuating Kitaev wires, a point of view adopted in \cite{tarantino2016,ware2016,kapustin2017}\dave{Need to double check the Kapstin ref, didn't realize he also did this. \ethan{I think it was more ragnar's point of view}}.
...
With the one-dimensional Hamiltonian at hand, we showed how to explicitly construct the MPS for the ground state wavefunctions in this graphical language, recovering the same MPS as \cite{fidkowski2011,turzillo2016,bultinck2017b}.\dave{will double check this.} 
...
and employing the shadow world construction outlined in Section~\ref{shadowworld}\dave{Will check that we all agree on this. \ethan{I do}}.
