...
\be\label{VIbetabeta}
 V(I;\beta,\beta) = \cc \left[ \halfchain\;, \; \halfchaindot \right].\ee
When we glue the end points of two $\beta$ strands into a single strand, 
we must do the gluing with the proper choice 
of tensor product to ensure that we don't produce any extra degrees of freedom. 
This is accomplished using the relative tensor product discussed in Section \ref{modified_tensor_product}, 
and ensures that
%We thus can write 
\be V(I;\beta,\beta) \cong V(I;\beta,\beta) \tp_{\End(\beta)} V(I;\beta,\beta).\ee
Recall the relative tensor product $\tp_{\End(\beta)}$ (a.k.a $\tp_{\cliff_1}$) is needed to mod out by local relations involving the sliding of fermions along $\beta$ lines, 
...
%as was discussed at length in Section \ref{modified_tensor_product}. 
Graphically, utilizing $\tp_{\cliff_1}$ is equivalent to performing the regular tensor product $\tp_\cc$ and modding out by the equivalence relations
\be
\label{graphical_equiv_reln} 
\halfchain \tp_\cc \halfchaindot \; =\; \halfchaindot \tp_\cc \halfchain\; \quad \quad \text{and} \quad \quad 
\halfchaindot \tp_{\cc} \halfchaindot \;  = -\lambda\; \halfchain \tp_{\cc}  \halfchain,
\ee 
where we have assumed a left-to-right Koszul ordering (in the notation of Table \ref{C2_data_table} we $\lambda = A^4$).
%\be
...
we can impliment these equivalence relations via a Hamiltonian.
The local Hilbert space is given by \eqref{VIbetabeta}, 
which for convenience we write as, 
\begin{align}
V(I, \beta, \beta) = \cc \left[ \LocalHilba, \LocalHilbb \right].
\end{align}
The total Hilbert space of the chain is given by,
\begin{align} 
\label{HilbInterval}
\mch_I  = V(I, \beta, \beta)  \tp V(I, \beta, \beta)  \tp \cdots \tp V(I, \beta, \beta) 
\end{align} 
The projector that mods out by the equivalence relations \eqref{graphical_equiv_reln}, 
is given by 
\begin{align} 
H = \frac{1}{2} \left(  \KitHama -  \KitHamb \right).
\end{align}
It couples together the nearest neighbour sites in $V(I, \beta, \beta)  \tp V(I, \beta, \beta)$ by straddling int he junction.
