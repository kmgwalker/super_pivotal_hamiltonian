...
either $\Sigma = I$, $\Sigma = S^1_B$, or $\Sigma = S^1_N$, 
\kw{in math papers, $\Sigma$ is almost always a 2-manifold}
where $I$ is an interval in $\mathbb{R}$. 
\kw{why does it need to be embedded in $\mathbb{R}$?}
The shadow world prescription tells us to examine 
partition functions on $\Sigma \times [0,1]$. 
\kw{I'm confused by the dimensions here.  Are we working with a 1+1-dim'l TQFT?  Do you really mean ``partition function"?}
We will specify to $\Sigma = I$ in what follows, and examine 
...
from the shadow world approach with some given vectors $v_0,v_1$. 
