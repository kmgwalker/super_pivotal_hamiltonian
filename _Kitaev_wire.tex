...
an idea previously investigated in \ref{tarantino2016,ware2016}.
...
\be V(I;\beta,\beta) = \cc \left[ \halfchain\;, \; \halfchaindot \right].\ee
When we glue two $\beta$ strands into a single strand, we must do the gluing with the proper choice 
of tensor product to ensure that we don't produce any extra degrees of freedom. We thus can write 
\be V(I;\beta,\beta) \cong V(I;\beta,\beta) \tp_{\End(\beta)} V(I;\beta,\beta),\ee
where the relative tensor product $\tp_{\End(\beta)}$ (a.k.a $\tp_{\cliff_1}$) is needed to mod out by local relations involving the sliding of fermions along $\beta$ lines, as was discussed at length in Section \ref{modified_tensor_product}. 
...
\be
\label{graphical_equiv_reln} \halfchain \tp_\cc \halfchaindot \sim \halfchaindot \tp_\cc \halfchain\;, 
\ee 
\be
